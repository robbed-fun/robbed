...
Copy `.env.example` → `.env`. Required: `KEEPER_RPC_URL`, `CHAIN_ID`, `KEEPER_PRIVATE_KEY`, `DATABASE_URL`. Startup fails closed on a missing/invalid var and on a chain-id mismatch vs the live RPC. Full table + operational notes in the runbook.
...
```
src/
  index.ts    entrypoint — wires detection + sweep + balance-watch + /healthz, graceful shutdown
