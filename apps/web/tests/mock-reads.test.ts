...
describe("mockCurveReads — Trust panel live rows (§5.2)", () => {
  ...
  it("row 3: live curve reserves render as the mockup values", () => {
    expect(formatEthFromWei(r.reserves.realEth)).toBe("52.7");
  });
  ...
  it("row 4: graduation progress = realEth / threshold ⇒ 62% (mockup BONDING 62%)", () => {
    ...
    expect(formatEthFromWei(r.graduationEth)).toBe("85");
  });
...
describe("mockLaunchEconomics — Create economics (§5.3, docs/Robbed.html §2b)", () => {
  ...
  it("Starting price = virtualEth0 / virtualToken0 ⇒ 0.000001 ETH", () => {
    const price =
      Number(formatEther(e.virtualEth0)) / Number(formatUnits(e.virtualToken0, 18));
    expect(formatEthNumber(price)).toBe("0.000001");
  });

  it("Graduation threshold = 85 ETH", () => {
    expect(`${formatEthFromWei(e.graduationEthWei)} ETH`).toBe("85 ETH");
  });
