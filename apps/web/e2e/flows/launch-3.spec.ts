...
// @flow:LAUNCH-3 — Economics panel display (LP copy verbatim + live reads) (§5.3)
...
test(
  ...
  async ({ page }) => {
    ...
    await assertUi("EconomicsPanel renders the exact shared LP sentence + live fee/threshold", async () => {
      // LP destiny copy is verbatim from the single shared constant (§12.14).
      ...
      // No hardcoded USD/mcap literal in the economics copy (§2). The dollar
