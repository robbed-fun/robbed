/**
 * V3 Collect handler (indexer.md, M2-5 sub-task 5e).
 *
 ...
 * `token_is_token0`. Feeds the treasury fee-accrual dashboard.
 *
 ...
 * treasury` alert via the pure `feeRecipientAlert` decision.
 */
...
ponder.on("V3PositionManager:Collect", async ({ event, context }) => {
  ...
  // Gate-7: a Collect to any non-treasury recipient pages immediately.
  const alert = feeRecipientAlert(recipient, config.treasury, {
    token: grad.tokenAddress,
    txHash: event.transaction.hash,
  });
  if (alert) {
    incFeeRecipientMismatch(); // gate-7 metric
  ...
  // Redis publish → treasury fee-accrual dashboard live update (X-6).
  publishFeeCollected({
    token: grad.tokenAddress,
    recipient,
    amountToken,
