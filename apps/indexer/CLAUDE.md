...
Ponder → Postgres (+pg_trgm) → Redis pub/sub → Bun WS fanout (sidecar). Spec §8; confirmation states §2.1 (soft-confirmed → posted-to-L1 → finalized).
...
- ABIs + event types come from `@robbed/shared` (committed codegen, §12.38) — never paste ABIs locally.
