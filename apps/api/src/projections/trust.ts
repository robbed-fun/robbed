/**
 * Trust panel projection (api.md). All derived, none hardcoded. The
 ...
 * `token_flow_stats`, null until computed; a RANGE, never a point value.
 *
 * `feePolicy.tradeFeeBps` is the PER-TOKEN `tokens.trade_fee_bps` snapshot
 * — the fee that curve actually charges — NOT the factory-current
 ...
 * cannot drift (anti-drift; api.md "same projection").
 */
export function buildOrganic(flow: TokenFlowStatsRow | null): OrganicFlow | null {
  return flow
    ? {
        ...
        methodology: "heuristic estimate — organic-flow scoring",
...
export function buildTrust(row: TokenDetailRow): TrustPanel {
  return {
    ...
    lpCopy: LP_COPY,
    feePolicy: {
      tradeFeeBps: row.trade_fee_bps, // per-curve snapshot, not factory-current
      creatorFeeBps: row.creator_fee_bps, // 0 in v1, present from day 1
    },
