 ...
 * (spec §12.30 / findings X-1; anti-drift extraction rule 3).
 */
import { z } from "zod";
 ...
 * §2.2). Zod's `.max()` counts UTF-16 code units, NOT bytes, and therefore
 ...
 * (§12.30). Making API-vs-chain drift impossible by construction: acceptance
...
export function byteBoundedString(maxBytes: number, label: string) {
  return z
    ...
    .refine((s) => utf8ByteLen(s) <= maxBytes, {
      error: `${label} must be at most ${maxBytes} UTF-8 bytes (on-chain bytes(x).length gate, §12.30)`,
    });
}
