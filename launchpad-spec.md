...
45. (2026-07-10) **Hosting split ratified (user-directed): backend on self-hosted Komodo, Next.js frontend on Cloudflare Workers via OpenNext.** Resolves the architecture.md §6 "Hosting" deferral ("concrete host/orchestration decided at M4") ahead of schedule by user direction. **Two targets:**
    ...
    - **Required frontend consequence — OG rendering moves off the native N-API path to WASM.** `workerd` cannot load native N-API addons, so `@resvg/resvg-js` (the native rasteriser chosen at implementation-plan **M3-8**) is unrunnable on the Workers target. The OG route must use a **WASM path**: `@resvg/resvg-wasm` + `satori`, or Next's `ImageResponse`/`next/og` (which ships a WASM resvg). **This supersedes the M3-8 native-`@resvg/resvg-js` choice *for the deploy target*** (web.md §6's "raw `satori` + `@resvg/resvg-js`" and `serverExternalPackages` native-dep note): the native dep is removed and replaced with the WASM rasteriser. Layout/`satori` code is unchanged; only the raster backend swaps. Owner: **hoodpad-frontend** (task spec below), executed after the concurrent `apps/web` redesign.
    ...
    **Scope note:** this is an infra/deployment-target decision. It changes no contract, indexer, or API *logic* — only where processes run and which OG raster backend ships. Bun remains the local runtime/test runner everywhere (§8/§9); the frontend still *builds* under the §12.37 Next.js 16 + React 19 pins. Amends architecture.md §6; supersedes implementation-plan M3-8's native-resvg raster backend for the Workers deploy target. Records the P-3 hosting split (indexer/api → Komodo Dockerfiles + Stack; web → Workers, no web Dockerfile). (hosting decision)
...
- M0 notebook outputs (curve constants + V3 graduation tick/sqrtPrice recorded in `constants.json`, still under the §13 M0-review gate): **anti-sniper window+cap values (O-7) RESOLVED (§12.32)**; **arb-back `TOLERANCE_TICKS`/`MAX_ARB_ITERATIONS`/`MIGRATION_SLIPPAGE_BPS` param values (O-8) RESOLVED (§12.33)** — the token-leg budget rule was ratified and the numbers are now set, **but the WETH-leg budget *definition* remains a hoodpad-contracts M1 obligation** (threat-model §8.1, gate-2 invariant 6); **graduation caller reward (O-9) RESOLVED (§12.34)** (re-validate ≥10× `graduate()` gas at M1). All three (O-7/O-8/O-9) are **M0 defaults, tunable in the capped beta**
...
- ETH/USD snapshot source on 4663 (Chainlink feed existence — check at M2 start; else DefiLlama/Coinbase, config-driven) — **disposition DECIDED (§12.48a):** config-driven `ETH_USD_SOURCE_URL`, Chainlink-if-present else DefiLlama/Coinbase; the **on-chain feed-existence verification is ENV-GATED (needs a live 4663 RPC)** and finalized at M2 start — DefiLlama/Coinbase is the always-available default until confirmed (indexer OI-6)
- Robinhood RPC `safe`/`finalized` block-tag support (M2 day 1; fallback = L1 rollup-contract watermarks) — **disposition DECIDED (§12.48b):** use tags if supported, else the pre-sanctioned L1-watermark fallback (M2-3b); the **tag-support verification is ENV-GATED (needs a live 4663 RPC)** and decided day-1 of M2 (indexer OI-8)
...
- Ponder external-write tolerance for `confirmation_state` materialization vs sidecar table — **RESOLVED (§12.48c):** sidecar `event_confirmations` join table is the sanctioned default (satisfies §2.1/§12.20 regardless of version); direct in-column UPDATE permitted only if the pinned Ponder version demonstrably tolerates it (VERSION-GATED, not RPC-gated) (indexer OI-11)
