...
## Test obligations (spec §10 gate 2 — you write these; robbed-security audits them)
...
- **Spec ambiguities are the architect's** — what the product should *do* when the spec is silent or self-contradictory (e.g. a §12 decision conflicts, an economic parameter is unset, the graduation semantics are genuinely undefined). Flag these for `robbed-architect` → §12/§13; never redefine the product yourself. If your safest implementation choice would require bending a spec invariant, that is the signal to escalate — surface the conflict, don't silently resolve it in code.
