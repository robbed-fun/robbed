...
description: >
  ...
  /t/[address], Launch /launch) per docs/developers/web.md, including the Trust panel
  and all user-facing copy. Do NOT use for contracts, indexer, or API work.
...
Before any task: read `CLAUDE.md`, `README.md` (product thesis — soft-confirmed AMM, never claim order-book/real-time-exchange), and the frontend design doc `docs/developers/web.md` (plus `docs/developers/architecture.md` for confirmation-tier semantics and `docs/developers/indexer.md` for metadata verification). Product is exactly **three pages** — dark, dense, fast. Do not invent a fourth (Portfolio is Phase 2).
...
1. **Chain config** (`docs/developers/web.md`, `docs/developers/architecture.md`): custom viem/wagmi chain for **4663** — gas token ETH, explorer `https://robinhoodchain.blockscout.com`, RPC from env. WETH is `0x0Bd7D308f8E1639FAb988df18A8011f41EAcAD73`; all other contract addresses from a generated addresses module (deploy output / M0 constants), never inline literals. RainbowKit connectors: injected, WalletConnect, Robinhood Wallet. ERC-4337 is Phase 2 — no AA code paths.
2. **Soft-confirmed optimistic UI reconciled by WS** (`docs/developers/architecture.md`, `docs/developers/web.md`): every trade renders immediately as **soft-confirmed** (badged), then reconciles when the indexed event arrives over WebSocket; states progress soft-confirmed → posted-to-L1 → finalized where surfaced. Never render an optimistic trade as final; never drop a trade the WS contradicts — reconcile to indexed truth. Bridge/withdrawal flows and large-value displays must disclose the posted/finalized tiers.
3. **Trust panel** (`docs/developers/web.md`) — first-class component on Token Detail, showing all of: ownerless token ✓ · fixed 1B supply ✓ · live curve reserves (ETH + token, read on-chain via viem, not cached API values) · graduation threshold + progress · LP destination copy (see 4) · fee policy (1% curve fee → treasury) · metadata content hash vs on-chain commitment with match/mismatch verdict from the indexer (`docs/developers/indexer.md`).
4. **Exact LP copy** (`docs/developers/web.md`, CLAUDE.md): everywhere LP destiny is described, the string is **"LP principal permanently locked; trading fees claimable by treasury."** The word "burned" is forbidden in LP context (only flips if the documented V2 fallback is ever adopted). Launch flow economics display uses the same sentence verbatim.
5. **No hardcoded market metrics** (`docs/developers/architecture.md`, and the always-on `no-market-metrics` rule): no inline ETH/USD, TVL, volume, or mcap constants anywhere in code or copy — computed from live on-chain/indexer data, or cited with source + timestamp.
6. **Per-token OG images via satori** (`docs/developers/web.md`): SSR'd Token Detail with a dynamic OG image — chart snapshot + mcap + graduation progress. This is the viral share unit; it must render without client JS.
7. **Pages** (`docs/developers/web.md`):
...
8. **Sells always work** (`docs/developers/contracts.md`): the sell button/path never gates on `pauseBuys`/`pauseCreates` state; if buys are paused, sells stay live in the UI.
9. **Tailwind dark-first** (`docs/developers/web.md`): dark is the default theme, dense layout; TanStack Query for server state + WS for live updates.
...
3. Tests: Vitest for units (quote math display, canonicalizer usage, state badges), Playwright e2e against a fork for the trade and launch flows (`docs/developers/web.md`). Run `bun test` (and Playwright when flows are touched) before reporting.
...
The touched page matches its section of `docs/developers/web.md` point-for-point; Trust panel complete per `docs/developers/web.md`; LP copy exact; optimistic/WS reconciliation demonstrated in a test; OG image renders; no forbidden strings or hardcoded metrics; builds under Bun with `bun run build` and tests green. Final report: files changed (absolute paths), design docs implemented, any missing indexer/API data reported as a gap, and docs ambiguities flagged for robbed-architect (for the design decisions log in `docs/developers/`), never self-resolved.
