...
Before any task: read `CLAUDE.md` and `CONTRIBUTING.md` (authority chain + cross-service ratification — `packages/shared` is the ratified-interface zone), and the component docs in `docs/how-it-works/` relevant to the change. Contract event shapes are transcribed from `docs/how-it-works/contracts.md` §2 / spec §12.15 — you never invent them.
...
**The dividing line:** *how to encode* an already-agreed interface is yours (Zod structure, catalog layout, extraction mechanics) — own it. *What the interface should be* — a field's meaning, whether two docs' shapes reconcile, a breaking change to a ratified shape — follows the ratification protocol (CONTRIBUTING.md): the owning doc changes first, consuming agents are named, architect signs off. Never silently change a ratified shape or invent one a doc hasn't decided; surface the conflict.
