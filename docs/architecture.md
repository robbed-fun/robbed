...
flowchart LR
    ...
    subgraph fe["Frontend"]
        WEB[Next.js 15 on Bun<br/>apps/web]
    end
...
4. **Web:** status pill flips, TradeWidget silently re-engines to Uniswap V3 (QuoterV2 + SwapRouter02), chart continues as **one series** — the pool was initialized at the curve's terminal price, so there is no economic or visual seam. Post-graduation no hoodpad contract has any pause authority.
...
| Deploy artifacts → generated addresses/ABIs | hoodpad-contracts (M1 output) → shared codegen | indexer config, web `lib/addresses.ts` (never hand-edited) |
...
| Cloudflare R2 + CDN | Content-addressed images (`images/{keccak}.webp`) and canonical metadata (`metadata/{hash}.json`) | R2 is mutable; the on-chain hash is the commitment; indexer re-verifies |
...
| Hosting | Bun processes: web (Next.js), API (Hono), WS fanout; Ponder in a Node container | API HTTP and WS fanout scale independently; concrete host/orchestration decided at M4 with gate-7 monitoring |
