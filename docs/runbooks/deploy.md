...
**Status:** v1.2, 2026-07-12. Authored by hoodpad-architect + hoodpad-contracts (implementation-plan **P-2**; the M4/M5 handoff register at the bottom is **P-4**). v1.1: §3.1 updated for the §12.49 domain decision; handoff register re-derived against spec §13 as of 2026-07-11 (OI-6/OI-8/OI-11/web-10 closed) and extended with the H.6 caps-lift residuals from the M1 close-out. v1.2 (staleness sweep): H.6 `PORT-*` row corrected to its ratified state (PORT-1..8 RATIFIED 2026-07-11, 44-flow I-5a baseline); H.6 mutation row re-verified against `contracts/reports/mutation/scores.tsv` (adequacy 0.800 post-rerun — already recorded correctly, no change). This is a **prepared, not executed** runbook — the Goal is "production-ready, not production-launched" (spec §14 Phase A). Nothing here runs until Gate G-A passes and the user directs a mainnet launch (Phase B / M4).
...
| ~~**`PORT-*` flow ratification**~~ **CLOSED 2026-07-11** *(row updated 2026-07-12 — was stale: "ratification pending")* | **PORT-1..8** authored (user-flows.md §3b addendum + 8 waiver rows) and **RATIFIED by the architect the same day** (sign-off line in the `docs/user-flows.md` header; advisory-read semantics ratified — decisions.md §13). **44 flows = the I-5a `e2e:coverage` baseline.** Row retained for register completeness only — nothing pending, never was a launch item. | closed (was: robbed-architect, pre-I-5a) |
