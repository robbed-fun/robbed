...
- Fund it at the faucet (section 3), then wire it into the root `.env` (section 4).
...
```bash
# ── identity ────────────────────────────────────────────────────────────────
DEPLOYER_PRIVATE_KEY=0x…       # from `cast wallet new`, faucet-funded (section 2.1, section 3)
...
| deploy the contracts | create the T-2 Safe (`bun run safe:create`), fill `treasurySafe` in `tools/m0/external.testnet.json`, re-run the testnet derive, then the section 6 forge command + `bun script/emit-testnet-env.ts` |
