...
**Status:** Design-time artifact, authored **before any code existed** so it cannot be biased by what got built. Root authority: the [root README](../../README.md) + these developer docs; hard rules in `CLAUDE.md`; ratified rulings in [design-decisions.md](design-decisions.md). When this doc and a design doc disagree on a *defense*, the design doc wins and this doc gets a PR; when this doc names a *threat the design docs do not cover*, that is a finding routed to the architect (section 8 of this doc).

**Adversarial stance.** This is a refutation document, not a defense catalog. Its job is to find where funds, integrity, or the headline guarantees can break. The most load-bearing product claim — *"sells are always open; no pause authority can block a curve exit"* (section 6.5) — is treated here as a hypothesis to be attacked, not an axiom. Section 4 and Section 6 record where that claim is in fact defeatable.
...
flowchart TB
    ...
    subgraph Z3["Zone 3 — Privileged keys (trusted, single largest blast radius)"]
        ...
        SIGN[Safe signer set M-of-N — open item]
...
| **Z4 external** | correct WETH/NPM/pool/precompile behavior at the addresses we integrate | addresses MUST come from official registries (the open items); startup fails if unset | a wrong/hostile address here = graduation mints into an attacker venue |
...
| **Z6 operator** | transaction inclusion, ordering (FCFS), soft-confirmation honesty, L1 posting | we submit txs and hope for inclusion | **no recourse**: censorship/reorder/equivocation are inherited, not mitigated, unless L1 force-inclusion exists (unverified, the open items) |
...
| A2 | **User trade value in flight** | tx from wallet → curve/pool | bad fill, sandwich, front-run, mis-set slippage from a lying quote | slippage+deadline on every path (section 6.5); FCFS; **but** quote source integrity (Z5/RPC) is unguarded |
| A3 | **LP principal in the vault** | `LPFeeVault`-held V3 NFT | principal extracted / liquidity decreased | vault has no `decreaseLiquidity`/`transfer`/owner (section 6.3.4); collect-only |
...
| A5 | **Graduation escrow (transient)** | curve→migrator during `graduate()`, pre-mint | hostile-ratio mint burns real ETH value into an attacker-skewed position | arb-back-or-revert, amount-mins, no-hostile-mint invariant (section 6.3.2, gate 2) |
| A6 | **Metadata / image integrity** | on-chain `metadataHash` + R2 JSON/image | displayed content diverges from the on-chain commitment (image-swap) | JSON hash verified vs chain (section 8.3); **image bytes NOT server-verified in v1 (OI-10)** |
...
| A8 | **Admin keys** (Safe signers, SIWE allowlist) | off-chain custody | compromise → A4 redirect of *future* fee sweeps + A1 buy-freeze; **sell-freeze via treasury pointer is closed (D-25)** | Safe M-of-N (open item); no on-chain upgrade/extract power by design (section 6.6) |
...
| **Malicious admin / compromised Safe signer(s)** | reach threshold on the Safe → `setTreasury`, pause creates/buys, retune future-curve fees | redirect *future* fee sweeps; freeze the platform's *buy* side; **can NO LONGER freeze sells** (D-25 pull-payment — no sell-path treasury call) | compromise M-of-N signers | A4, A1 (buys) |
| **Compromised sequencer / Robinhood operator** | censor, reorder within FCFS, equivocate on soft-confirmations, downtime | censor a sell, self-sandwich, roll back a soft-confirmed trade | operator-level; no external cost | A2, sell-liveness (b/c of section 0) |
...
### 3.1 Observed adversaries (v1.2, section 2.2 — these are DEPLOYED, not theoretical)

Manual mainnet tx analysis (2026-07-09, re-verify at M2 with own indexer section 8.5) confirmed the sniper/wash/multi-wallet cohorts above are **live on-chain**, not hypothetical:
...
- **Gas-funder farm** — an EOA (`0x1887FA…`, ~1084 ETH) drip-funds ~0.0004 ETH to fresh wallets every 1–2 min → **empirically confirms the multi-wallet-bypass** of the per-tx anti-sniper cap acknowledged in section 6.5 (the guard blunts single-tx sweeps only; per-actor evasion via wallet rotation is real).
- **Programmatic majority** — >50% of sampled swaps have `sender ≠ recipient` → chain-level activity metrics are inflated (the binding organic-flow discount, section 2.2).

**Gate-6 red-team scenarios are parameterized against these three (section 10 gate 6):** (a) **multi-wallet sniping from a shared gas-funder** — simulate a funder fanning out to K wallets each buying `MAX_EARLY_BUY` in the early window, and **quantify the total cost and fair-launch distortion** (the multi-wallet bypass is now a measured cost, not an assumption); (b) **same-second multi-pool exits** — an arb executor draining WETH from ≥3 graduated pools in one block, verifying no curve invariant breaks and the indexer's `arb/exit` labeling (section 8.5) is correct; (c) **wash-loop volume** — self-trading clusters inflating `vol24h`, verifying the ~2% round-trip fee brake (Section 6, R10) and that section 8.5 excludes wash volume from organic metrics. Gate 6 reports the attacker's minimum spend for each.
...
| T (tamper) | Caller supplies a fee amount/recipient | **M**: no fee param exists in any signature; fees in-contract only (section 4.1, CLAUDE.md hard rule) |
| T | Reentrancy via ETH `call` to trader/treasury/refundTo | **M**: `nonReentrant` on all state-mutating externals + CEI; reentry hits guard or terminal phase (section 5.4). *Gate-2 invariant: no-ETH-extraction-beyond-fair-value.* |
| R (repudiation) | — | events on curve/factory; indexer records |
| I (info) | — | all data public |
| D (DoS) | `sellWithPermit` griefed by front-run permit consumption | **M**: try/catch proceeds if allowance suffices (contracts.md section 5.7) |
...
| E (elevation) | Router holds/strands ETH an attacker reclaims | **M**: Router has no `receive()`, never custodies ETH; curve pays recipients directly (contracts.md section 2.4) |

### 4.2 BondingCurve
| STRIDE | Threat | Disposition |
|---|---|---|
| T | Rounding-direction extraction (buy/sell math skims value) | **M**: buy `ceilDiv` favors protocol, sell rounds ethOut down (section 6.2). *Gate-2 invariants: k non-decreasing; no-extraction-beyond-fair-value.* |
| T | Fee off-by-one / drift vs treasury receipts | **M**: single fee site per leg; *gate-2 exact-fee-accounting invariant (== to the wei)*. |
| T | Insolvency — pay out ETH the curve doesn't hold | **M**: *gate-2 solvency invariant `balance ≥ realEthReserves`*; donations swept not credited (contracts.md section 5.7). |
| T | Hostile-ratio value creation at graduation boundary | **M**: graduation clamp lands exactly on `GRADUATION_ETH`, excess refunded, `minTokensOut` still honored (section 6.2). *Gate-2: graduation single-fire & reachable.* |
| S | Trade functions called by non-Router | **M**: `onlyRouter`; `graduate()` intentionally permissionless + `nonReentrant`. |
| T | `block.number`-based logic (L1 estimate on Orbit) | **M**: banned; anti-sniper uses `block.timestamp`; CI grep (section 5.1, hard rule). |
| D | Sell path DoS via treasury that reverts on ETH receive | **UM-1 — RESOLVED (D-25):** fixed by pull-payment fee escrow — the sell (and buy) path never calls the treasury; the fee accrues to `accruedFees` and is withdrawn by a permissionless, non-phase-gated `sweepFees()`. A reverting treasury reverts only `sweepFees()` (retriable), never a sell. Verified by the reverting-treasury sell case in the gate-2 pause matrix + solvency drain (contracts.md section 6). |
| D | **Curve funds frozen in `ReadyToGraduate`** if graduation is grief-locked | **UM-2** (High): both directions lock in that phase (D-12); if `migrate()` is forced to persistently revert, ETH is stuck → defeats section 0(a). |
| D | Anti-sniper single-tx sweep | **M (bounded)**: per-tx `MAX_EARLY_BUY` in early window (section 6.5); **multi-wallet bypass acknowledged** — quantify cost in gate 6. |
...
| STRIDE | Threat | Disposition |
|---|---|---|
| T | Pre-seeded pool at hostile price → hostile-ratio mint | **M**: `initializePool` at creation; `migrate` never trusts `slot0`, arbs back or reverts (section 6.3.2). *Gate-2: donation + sync-style + swap griefing fuzzed → corrected-or-revert, never hostile mint.* |
| T | Reentrancy via `uniswapV3SwapCallback` from a fake pool | **M**: callback reverts `NotPool` unless `msg.sender == _activePool` for the in-flight migration (contracts.md section 2.5). |
...
| S | Wrong V3 Factory/NPM address (invented, not registry) | **M**: addresses from official registry only (the open items/O-4); startup + fork tests fail if wrong. |
...
| E | Migrate callable by non-curve | **M**: `onlyCurve` via `factory.isCurve` (contracts.md section 2.5). |

### 4.4 LPFeeVault
| STRIDE | Threat | Disposition |
|---|---|---|
| E | Principal extraction / liquidity decrease | **M**: no owner, no `decreaseLiquidity`/`approve`/`transfer` initiation; sole external fn `collect()` → fixed treasury (section 6.3.4). Auto-fail on any added privileged path. |
...
| STRIDE | Threat | Disposition |
|---|---|---|
| E | Owner reaches into live-curve economics | **M**: economics snapshotted per curve at creation; owner setters affect future curves only; unit-tested negative property (contracts.md section 7.3). |
| E | Owner pauses sells | **M**: no `pauseSells` flag exists, **and** the treasury-pointer backdoor (former UM-1) is closed by D-25 — the sell path never calls the treasury (pull-payment), so pointing `treasury` at a reverter cannot block sells. |
...
| D | Beta caps mis-accounted, blocking buys | **M**: `recordEthDelta` decrements on sells *and graduation* (section 2.2 line: "sells/graduation delta negative, never reverts"); global sum tracks live curve ETH. Sells never revert on cap (floor-at-zero). |
| E | Proxy/upgrade sneaks in | **M**: immutable, no proxies; upgrade = new factory (section 6, auto-fail on violation). |
...
| T | Reorg rolls back soft-confirmed trades inconsistently | **M**: watermarks only reference L1-posted blocks; rolled-back events were `soft_confirmed` by definition; `reorg` message drops orphans (section 5.3). |
...
| I | Pre-grad pool activity leaks into price series | **M**: pre-grad V3 activity not indexed; curve is sole venue until `Graduated` (D-16). |

### 4.7 API / moderation
| STRIDE | Threat | Disposition |
|---|---|---|
| T | Unmoderated bytes reach CDN (polyglot/CSAM) | **M**: API-mediated upload, MIME magic-byte sniff + re-encode before R2 (section 8.4, D-19). |
...
| E | Admin escalates beyond hiding a listing | **M**: `/v1/admin/*` can only set listing visibility/impersonation flags; no chain-affecting path exists (section 8.4). |
| S | Admin session forgery / CSRF | **M**: SIWE against allowlist, HttpOnly SameSite=strict cookie + CSRF token (section 6.2). Residual = allowlist/signer-set (the open items). |
| D | Search/storage/SSRF DoS | **M**: trgm floor + statement timeout, size caps + orphan sweep, no server-side fetch of user URLs (section 6.3). |
| I | Impersonation token evades the watchlist | **UM-7** (Low/Med): watchlist curated + refreshed only ≥ monthly; the WS launch ticker is **unmoderated** in v1 (D-21) — new impersonations surface immediately, unflagged for up to a refresh cycle. |
...
| S | Optimistic row spoof / never-reconciled ghost trade | **M**: optimistic rows reconcile to indexed truth by `txHash`, never trusted-as-final, never silently promoted (web reconciliation section ; section 2.1 badges). |
...
|---|---|---|---|
| **Censorship** | sequencer may drop/refuse any tx, including a **sell** | nothing at the contract layer; "no `pauseSells` flag" does not bind the sequencer | **Sell-liveness section 0(c) is void under censorship** unless Orbit L1 delayed-inbox / force-inclusion is available on 4663 — **unverified (the open items)**. This is the single most important open assumption behind the exit guarantee. |
| **Reordering within FCFS** | priority fees do NOT jump the queue (section 2) — this *reduces* generic sandwich MEV | rely on FCFS + per-tx slippage/deadline | the sequencer itself can reorder/insert; a compromised sequencer can self-sandwich with no fee signal. Not externally detectable. |
| **Downtime** | no trades/exits while the sequencer is down | none | availability risk; funds not lost but exits paused for all. |
| **Soft-confirmation reversal** | a soft-confirmed trade can vanish before L1 posting (equivocation or deep reorg) | confirmation tiers (soft→posted→finalized) surfaced in UI; large-value flows disclose posted/finalized (section 2.1) | a user may act on a soft-confirmed state the operator later drops; `reorg` messages remove orphans but value acted-upon off-platform is unrecoverable. |
| **Settlement / challenge period** | L1 finality + challenge window for withdrawals | bridge/withdrawal UX discloses tier (section 2.1) | standard optimistic-rollup withdrawal delay; out of our control. |
...
- **R1 — No firm audit at launch.** Assurance = AI pipeline + capped beta + public bounty + an explicit external-review decision gate (section 10). No Sherlock/Code4rena contest is guaranteed pre-caps-lift; that decision is gate 9.
- **R2 — Single sequencer.** Robinhood operates the only sequencer. Censorship, reordering, downtime, and soft-confirmation reversal are inherited (Section 5). Curve-exit liveness depends on sequencer inclusion; L1 force-inclusion availability is unverified (the open items).
...
- **R5 — Admin (Safe) powers.** The factory owner can pause creates/buys, retune future-curve fees within caps, and repoint `treasury`. **The treasury-pointer sell-freeze (former UM-1) is closed by D-25 pull-payment:** no trade path calls the treasury, so a malicious/compromised/misconfigured treasury can no longer freeze sells — it can at most (a) revert its own `sweepFees()` (retriable; funds safe in-contract) and (b) redirect *future* fee sweeps. Buys remain pausable by design. Safe M-of-N and signer set are open (the open items).
...
- **R12 — Supply-chain: compiler pin + vendored math.** Exact pin `0.8.35` is unverified against the Blockscout verifier (the open items/O-5); vendored `TickMath`/`FullMath` ports are graduation-critical (UM-10).
...
| Anti-sniper cap + multi-wallet bypass cost (4.2) | 6 (sniper sim, **parameterized vs the observed shared-funder farm section 2.2/section 3.1** — bypass now empirically confirmed, sim quantifies cost) | UNVERIFIED |
| Sandwich residual under FCFS (A2) | 6 (sandwich sim) | UNVERIFIED |
| Wash-trade / trending gameability, R10 (4.7) | 6 (wash sim, **parameterized vs observed wash-loop volume section 2.2/section 3.1**; section 8.5 excludes wash from organic metrics) | UNVERIFIED |
| Same-second multi-pool arb exits (section 3.1) | 6 (arb-exit sim); section 8.5 `arb/exit` labeling | UNVERIFIED |
| **UM-1 treasury-set sell-freeze — RESOLVED (D-25 pull-payment)** (4.2/4.5/R5) | 2 (pause matrix now includes the reverting-treasury sell case + reverting-treasury solvency drain — contracts.md section 6), 7 (kill-switch inspection) | UNVERIFIED (design fixed; test added to gate-2 — the section 7 "needs an explicit test" gap is closed by the new pause-matrix case) |
...
| **UM-3/R2 sequencer censorship vs sell-liveness** (section 5) | 6 (assumption test), 10 (disclosure) | UNVERIFIED — **gap: force-inclusion unconfirmed** |
| **UM-4/R7 single-RPC integrity** (4.8/4.6) | 6, 10 (disclosure); no code gate | UNVERIFIED — **infra, no current gate** |
| **UM-5 stored-link XSS/phishing** (4.8) | (web e2e/CSP — not a section 10 gate) | UNVERIFIED — **gap: no section 10 gate owns this** |
...
The rows above omitted ~1/3 of the section 4 STRIDE threats; gate authors (5/6/10) must not skip them. The previously-unmapped threats and their verifying gate:

| Threat / finding (section 4) | Verifying gate(s) | Status |
|---|---|---|
...
| **Cross-entrypoint reentrancy** (refund→`graduate()`/`sweepFees()`) (section 8.1) | 2 (nonReentrant + CEI units, invariant 7) | UNVERIFIED — **M1 obligation** |
...
| Reorg rollback of soft-confirmed (4.6 T) | M2 confirmation/reorg unit suite (indexer.md section 5.3/section 9.1) | UNVERIFIED |
...
| Unmoderated bytes reach CDN (4.7 T) | M2 upload hostile-fixture suite (api.md section 8) | UNVERIFIED |
...
All statuses UNVERIFIED (design-time). Gate-5 (LLM) and gate-6 (red-team) prompt sets are derived from the **full** section 4 enumeration, not only the rows above the addendum.

---

## 8. Findings routed to hoodpad-architect (disposition required)

Ordered by severity. UM-1, UM-2, UM-3 are the ones that erode the section 0 headline guarantee and should be dispositioned before M1 freeze.

- **UM-1 — Critical — RESOLVED (D-25, 2026-07-09) — Treasury-pointer sell-freeze backdoor.** *Original finding:* every curve sell paid the fee to the factory `treasury` via a revert-on-failure `call` before paying the seller, so an owner/compromised-signer/buggy-module could freeze all sells by pointing `treasury` at a reverter — a de-facto `pauseSells`. *Disposition:* **pull-payment fee escrow** (candidate (a) — the cleanest). `BondingCurve` never calls the treasury on any trade path; the 1% fee accrues to `accruedFees` and is withdrawn by a permissionless, non-phase-gated `sweepFees()` (mirrors `LPFeeVault.collect()`). Solvency invariant → `balance ≥ realEthReserves + accruedFees`; exact-fee → `treasuryReceipts + accruedFees == Σ fees`. Sells are now un-freezable **by construction**; section 0(b) treasury dependency eliminated. **New M1 obligation on hoodpad-contracts:** implement `sweepFees()` + `accruedFees`, add the reverting-treasury sell test to the gate-2 pause matrix and the reverting-treasury solvency-drain (contracts.md section 6). The migrator's flat graduation-fee push to treasury still exists (graduate() is not a sell path) — see UM-2.

- **UM-2 — High — DISPOSITIONED (2026-07-09) — Graduation-lock fund freeze.** `ReadyToGraduate` locks both directions (D-12); a pool grief that keeps `migrate()`'s bounded arb-back from reaching `target ± TOLERANCE_TICKS` reverts `graduate()` (`PoolPriceUnrecoverable`) with real ETH inside; a reverting treasury on the graduation-fee push (still a push — D-25 scopes to trade paths only) is a second lock vector. *Disposition (architect):* **(i)** gate 6 MUST compute and prove a bounded griefing-cost analysis — the attacker's minimum spend to sustain the lock vs `GRADUATION_ETH` — documented in the gate-6 register; the arb-back captures attacker-donated funds, so griefing is expected to be strictly money-losing, but this must be *proven*, not asserted. **(ii) M1 design task for hoodpad-contracts (decide-it-yourself protocol):** design and prove either (a) that the frozen state is unreachable within the arb budget, **or** (b) a **bounded-retry + permissionless escape hatch** (e.g., after a timestamp timeout, a permissionless path that re-opens sells / re-attempts into a corrected pool) — hoodpad-contracts owns the mechanism choice and records it in contracts.md, escalating to the decision record only if it changes a product guarantee. **Flag:** if the cost analysis in (i) cannot show griefing is always uneconomic, the escape hatch (ii-b) becomes mandatory before caps-lift. For the graduation-fee-push leg, also evaluate accruing the graduation fee (pull) rather than pushing, so a reverting treasury cannot block `graduate()` either.

- **UM-3 — Medium(High if no force-inclusion) — Sequencer censorship defeats sell-liveness.** The contract-level exit guarantee is meaningless if the single sequencer censors the sell tx (section 5). Whether 4663 exposes L1 delayed-inbox/force-inclusion is unverified (the open items). Disposition: verify force-inclusion at M1/gate-6; state the true dependency in gate 10.
...
- **UM-1 → RESOLVED** — pull-payment fee escrow (D-25). See the UM-1 entry above.
...
This is a living design artifact. Every contract diff review (per the adversarial protocol) that surfaces a new attack class appends a threat here first, then a test. When an UM-n is dispositioned, record the resolution in [design-decisions.md](design-decisions.md) and flip its Section-7 status once the verifying gate passes. When a design doc changes a defense, this doc gets a PR — the design doc wins.
