...
**Spec coverage:** section 2 (no hardcoded market metrics), section 2.1 (confirmation semantics), section 5.1/section 5.2 (data the frontend consumes), section 7 (creator tracking from day 1), section 8 (off-chain architecture), section 8.3 (metadata integrity), section 10 gate 7 (monitoring), the open items (open items honored, not resolved here).
**Runtime:** Ponder in a Node container (section 8) → Postgres (+`pg_trgm`) → Redis pub/sub. Upstream RPC: Alchemy WS. Chain ID 4663.
...
1. Indexes the six event families (section 8, D-15–16): `TokenCreated`, `Trade`, `Graduated`, LaunchToken `Transfer`, V3 `Swap`, V3 `Collect`.
2. Maintains **venue-continuous** price/candle series per token across graduation (section 5.2, section 8) — one unbroken series from curve trades into V3 swaps.
3. Tracks **confirmation state** per indexed event: `soft_confirmed` → `posted_to_l1` → `finalized` (section 2.1) — derived at read time from the `confirmation_watermarks` sidecar (section 3.8/section 5, D-48c; per-row storage on Ponder tables is impossible, OI-11).
4. Verifies metadata integrity: R2 JSON vs on-chain `metadataHash` commitment (section 8.3) — feeds the safety strip (section 5.2, D-57; formerly the Trust panel).
5. Maintains holder balances (portfolio-ready from day 1, section 5.4) and `creator` / `creatorFeeBps` per token from day 1 (section 7).
6. Publishes every indexed event to Redis for the Bun WS fanout with a <500ms event-to-browser target (section 8).
7. Feeds the treasury fee-accrual dashboard from V3 `Collect` (section 6.4 post-graduation revenue, section 8).

Non-goals: the indexer never writes chain state, never gates listings by chain-side action (section 8.4 — moderation is listing-only and lives in the API), and never computes USD figures from inline constants (section 2 — ETH/USD comes from the `eth_usd_snapshots` table, sourced live and timestamped).
...
V3_FACTORY_ADDRESS        OPEN ITEM the open items — from official Uniswap registry at implementation time; startup MUST fail if unset
V3_NPM_ADDRESS            OPEN ITEM the open items — NonfungiblePositionManager, same rule
...
ETH_USD_SOURCE_URL        HTTP fallback price source for snapshots (DefiLlama/Coinbase; see section 3.9)
CHAINLINK_ETH_USD_FEED    D-51 Chainlink ETH/USD proxy; default 0x78F3556b67E17Df817D51Ef5a990cDaF09E8d3A9; 'off' disables (LOCAL/TESTNET)
ETH_USD_POLL_INTERVAL_MS  poller cadence, default 30000 (section 3.9 band 30–60s)
ETH_USD_CHAINLINK_STALENESS_SECONDS  reject feed answers older than this, default 3600 (section 3.9)
...
Canonical shape (D-15, contracts.md section 2.2):
```solidity
event TokenCreated(
...
Initial `virtual_eth`/`virtual_token` and `graduation_eth` are read **per-curve at `TokenCreated`** from the freshly-deployed `BondingCurve`'s public immutables (`VIRTUAL_ETH_0`/`VIRTUAL_TOKEN_0`/`CURVE_SUPPLY`/`LP_TOKEN_TRANCHE`/`GRADUATION_ETH`, plus `TRADE_FEE_BPS` — `src/curveReader.ts`) via the **shared read-function ABI in `packages/shared/src/abi/` (D-38)**; creation is low-frequency, so a bounded set of reads per creation is acceptable and there is NO hot-path RPC. **As-built note (2026-07-12, robbed-indexer):** there is no startup factory-level defaults read — the per-curve read is the sole source, because factory-current values would misreport any curve created under prior parameters (D-40d applies to the whole curve shape, not just the fee). The factory's `curveDefaults()` view (D-39 amendment; curve shape split out of `config()`, which now carries only the 12 operational fields) exists in the regenerated shared `curveFactoryAbi` but is **unused by the indexer** — it reports *future-curve* defaults, the wrong source for indexing existing curves. **The per-curve read supersedes the env-interim curve-constants source** used before the shared read-ABI landed (M2-4 note); the read ABI is a compilation-time artifact (needs only `forge build`, no deploy — contracts.md section 7.4). Block timestamp comes from the log's block. The creator's initial buy (if any) arrives as the first `Trade` in the same tx (D-15 — not in this event; capture path + fallback in section 7.4). `v3_pool_address` is populated here from `pool` (the pool exists pre-graduation by design), but V3 `Swap` indexing still begins only at `Graduated` (D-16).

**Per-token `trade_fee_bps` (D-40d).** `setTradeFeeBps` governs future curves only, so the live factory config would misreport the fee of any curve created under a prior fee. At `TokenCreated` the handler reads the curve's public `TRADE_FEE_BPS` immutable (via the D-38 shared read-ABI — creation is low-frequency, not a hot path) and stores it as `tokens.trade_fee_bps`, a per-token immutable snapshot. This column is the source for the API `trust.feePolicy.tradeFeeBps` (api.md section 3.4) — never the factory config. **Wiring follow-up (2026-07-10, decisions section 7):** the shared `db-rows.ts` `TokenRow` type carries `trade_fee_bps: number`, and the API card/detail projection reads it from this column — **not** from `apps/api/src/config.ts` (config is factory-current and would misreport older curves). Owner: **hoodpad-shared** (type) + hoodpad-indexer/API (wire).
...
CREATE TABLE tokens (
  ...
  creator              text NOT NULL,              -- section 7: day 1, even though creator fees are Phase 2
  creator_fee_bps      integer NOT NULL DEFAULT 0, -- section 7: 0 in v1; column exists so Phase 2 needs no migration
  trade_fee_bps        integer NOT NULL,           -- D-40d: per-token immutable snapshot, read from curve TRADE_FEE_BPS at TokenCreated (NOT factory config); safety-strip source (D-57)
  name                 text NOT NULL,
  ticker               text NOT NULL,              -- ≤10 chars enforced upstream; indexer stores verbatim
  metadata_hash        text NOT NULL,              -- bytes32 hex, verbatim from chain (section 8.3)
  ...
  -- live curve state (updated on every Trade; read by the safety strip section 5.2, D-57)
  ...
  v3_pool_address      text,                       -- set at TokenCreated (pool in event, D-15); Swap indexing starts at Graduated (D-16)
  graduated_at         bigint,
  -- denormalized market stats (derived, rebuildable; section 5.1 card + sorts)
  ...
  -- confirmation tier: NOT stored (OI-11/D-48c) — derived at read time from
  -- block_number vs the confirmation_watermarks sidecar (section 3.8, section 5)
  UNIQUE (tx_hash, log_index)
);

-- section 5.1 search: pg_trgm over name, ticker, contract address, creator address
...
Notes:
- `graduation_eth` is read from factory constants (event or contract read at handler time), never hardcoded (section 6.4 constants are deploy-time).
- Mcap/price are **never stored in USD**: USD is computed at query/render time as `price_eth × eth_usd_snapshot` (section 2, hard rule). See section 3.9.
- `image_url`/`description`/`links` are populated **only** from metadata JSON that has been fetched (any verification verdict); the safety strip (D-57) decides how to badge mismatches. Listing visibility is the API's moderation concern, not the indexer's.

### 3.2 `Trade` (emitted by each BondingCurve)

Canonical shape (D-15, contracts.md section 2.3) — emitted **on the curve** (curves registered as Ponder factory children via `TokenCreated`; `token` derived from the emitting curve address, not an event field):
```solidity
event Trade(
  address indexed trader,
  bool    indexed isBuy,
  uint256 ethAmount,           // GROSS ETH leg (fee included); net = ethAmount − fee (D-15)
  uint256 tokenAmount,
  uint256 fee,                 // ETH-leg fee → treasury (section 6.4), computed in-contract (section 4.1)
  uint256 virtualEthReserves,  // post-trade reserves → exact price with zero RPC reads
  uint256 virtualTokenReserves,
  uint256 realEthReserves
);
```
Block timestamp from the log's block. Token status for the API is derived, not stored: `curve` while trading, `graduating` when `real_eth_reserves ≥ graduation_eth AND NOT graduated` (the D-12 lock window), `graduated` after `Graduated`.

Derived records: `trades` row; `tokens` live-state update (reserves, last_price, counters); `balances` **cost-basis columns only** for the trader (see balance-write ownership below); candle upsert into all intervals (section 4); WS publish to `token:{addr}:trades` + `global:trades`.
...
**Balance-write ownership (X-4, ratifies D-16).** The `balances.balance` column and `tokens.holder_count` are written **only** by the `Transfer` handler (section 3.6 — the sole source of balance truth). The `Trade` and V3 `Swap` handlers **never** touch `balance`/`holder_count`; they write only the cost-basis columns (`total_bought_tokens`, `total_sold_tokens`, `total_eth_in`, `total_eth_out` — exact for curve legs, best-effort for V3 per OI-5). This removes the former double-count between the trade/swap handlers and the Transfer handler.
...
CREATE TABLE trades (
  ...
  venue                text NOT NULL DEFAULT 'curve' CHECK (venue IN ('curve','v3')),  -- unified with V3 swaps, section 3.4
...
CREATE INDEX trades_trader_idx       ON trades (trader, block_timestamp DESC);   -- Phase-2 portfolio ready (section 5.4)
CREATE INDEX trades_block_number_idx ON trades (block_number);                    -- `since` backfill / range reads
```

**Design decision — unified trades table:** curve `Trade` and V3 `Swap` both insert into `trades` with a `venue` discriminator. This is what makes venue continuity (section 5.2) structurally trivial: the candle pipeline and trade feed read one table ordered by `(block_number, log_index)` and never special-case graduation. *Interpretation, recommended default* — the spec says "one series" but not one table; a two-table union view would also satisfy it at higher complexity for zero benefit.

Price derivation:
- Curve: `price_eth = virtualEth_post / virtualToken_post` (constant-product spot after the trade). Post-trade reserves are in the event (ratified D-15) — no contract read in the hot path.
- V3: `price_eth` (WETH per token) `= f(sqrtPriceX96)` from the `Swap` event, adjusted for token0/token1 ordering vs WETH (both 18 decimals). The raw ratio `(sqrtPriceX96/2^96)^2` is **token1 per token0**. Since we want WETH per token: when the token is **token0** (WETH is token1, i.e. `token < WETH`) the raw ratio is already WETH-per-token — **use it directly**; when the token is **token1** (WETH is token0, i.e. `token > WETH`) the raw ratio is token-per-WETH — **invert it** (`1 / raw`). Orientation = `graduations.token_is_token0` (`token < WETH`), resolved per-pool at graduation and cached; the fork test (contracts.md section 6 gate 3) arbitrates the sign. *(Prior text said "inverted if token is token0" — that was backwards.)*

### 3.3 `Graduated` (V3Migrator)

Canonical shape (contracts.md section 2.5):
```solidity
event Graduated(
  address indexed token,
  ...
  address caller,              // permissionless graduate() caller (section 6.2)
  uint256 callerReward,
  uint256 tokensBurned,        // token dust → 0xdEaD (D-13)
  uint256 wethDustToTreasury   // WETH dust → treasury (D-13)
);
```
`token_is_token0` is derived (`token < WETH` address comparison), cached on the row.

Derived records: `graduations` row; `tokens.graduated = true`, `v3_pool_address`, `graduated_at`; **dynamic registration of the pool for V3 Swap/Collect indexing** (Ponder factory pattern over discovered pool addresses — see section 7.4); WS publish `token:{addr}:events` type `graduated` + `global:launches`.

```sql
CREATE TABLE graduations (
  token_address        text PRIMARY KEY REFERENCES tokens(address),  -- single-fire (section 10 gate 2 invariant)
...
Consumed: `event.transaction.from` (→ `trader`; the tx-sender EOA that submitted the swap — for a direct SwapRouter02 swap this is the user's EOA, correct for both buys and sells. **D-75 supersedes** keying `trader` on `recipient`: for an ETH-output sell `Swap.recipient` is the router — not the seller — which mis-attributed the trade to SwapRouter02 in the Token Detail Trades table. Contract-mediated / aggregator swaps attribute to the relayer — best-effort, Phase-2 4337; still flagged in feed UI as venue=v3), `amount0/amount1` (signs give direction; mapped to `is_buy` = pool paid out token to the swap output side), `sqrtPriceX96` (→ `price_eth`). Derived records: `trades` row with `venue='v3'`, `fee_eth=0`; `tokens.last_price_eth` + `volume_eth_24h`; candle upsert; **cost-basis columns only** for the `event.transaction.from` holder (best-effort — D-75, closes OI-5); **`balance`/`holder_count` are NOT written here** — the `Transfer` handler owns them (X-4 balance-write ownership, section 3.2/section 3.6); WS publish same channels as curve trades.

Only pools registered in `graduations` are indexed (Ponder factory config keyed on `Graduated`). Pools exist pre-graduation (section 6.3 pre-seed defense) — **pre-graduation pool activity is not part of the price series and is not indexed** (ratified D-16: the curve is the sole venue until `Graduated`; pool griefing is covered by gate-7 alerting on migrator events, not by indexing anomaly swaps).
...
```
Indexed on the NonfungiblePositionManager (address from config, the open items), filtered to `tokenId ∈ graduations.lp_token_id`. Feeds the treasury fee-accrual dashboard (section 6.4, section 8).
...
CREATE TABLE fee_collections (
  ...
  lp_token_id          numeric(78,0) NOT NULL,
  recipient            text NOT NULL,              -- must equal treasury; alert if not (monitoring section 8 of this doc)
...
CREATE INDEX balances_top_holders_idx ON balances (token_address, balance DESC);  -- top-20 (section 5.2)
CREATE INDEX balances_holder_idx      ON balances (holder);                        -- portfolio lookup (Phase 2)
```

Holder distribution flags (creator/curve/vault, section 5.2) are computed at query time by joining against `tokens.creator`, `tokens.curve_address`, and the vault/pool addresses — not stored.

`tokens.holder_count` maintained incrementally (balance transitions 0→positive / positive→0).

### 3.7 Candles (derived, rebuildable — section 4)
...
### 3.8 Confirmation tracking **[offchain]** (section 2.1, section 5)
...
### 3.11 Moderation flags **[offchain]** (section 8.4 — written by the API, read by both)

Owned by the API service (see `docs/developers/api.md` section 4); listed here because token list queries join it. The indexer never writes it. Listing gating = `WHERE moderation.visibility != 'hidden'` applied in API list/search endpoints — chain state and raw indexed data are never touched by moderation (section 8.4).
...
CREATE TABLE moderation_status (
  ...
  impersonation_flag   boolean NOT NULL DEFAULT false,   -- top-asset / Stock Token ticker match (section 8.4)
...
### 3.12 Creator-fee claimable roll-up (section 7 / D-63 — Phase-2, additive)
...
**Optional CreatorVault source:** the `CreatorVault` Ponder source is registered **only** when `config.creatorVault` resolves (env `CREATOR_VAULT_ADDRESS` or `getDeployment(chainId).robbed.creatorVault`, both optional). On a treasury-only deployment the vault source and its two handlers are simply not registered — the indexer runs unchanged (graceful skip, mirroring the D-55 optional-config gate). `tokens.creator_fee_bps` is populated per-token from the curve's immutable `CREATOR_FEE_BPS` at `TokenCreated` (read defensively → `0` for v1 curves that predate the leg).
...
### 3.13 Post-graduation 50/50 split roll-up (D-69 — Phase-2, additive)

The POST-GRAD half of the creator leg (section 3.12 is the pre-grad native-ETH half). The graduated V3 pool's 1% fees are split 50/50 creator/treasury at `LPFeeVault.collect(tokenId)` (`creatorLpShareBps() == 5000`); the treasury share is PUSHED to the fixed treasury, the creator share routed to the pull-payment `CreatorVault` as a per-`(creator, ERC20-token)` balance via `depositERC20(creator, token, share)` — Option B (D-69(C)). `token` ∈ {graduated launch token (sell-leg), canonical WETH (buy-leg)}; **not** unwrapped to ETH. Claimed per ERC20 via `claimERC20(creator, token)`; read live via `tokenBalanceOf(creator, token)`. Same reorg-tracked-Ponder-table rationale + additive/empty-on-v1 treatment as section 3.12. The pre-grad native-ETH balance stays in `creator_claimable` (separate).
...
**Authoritative claimable:** the API serves the **live** `CreatorVault.tokenBalanceOf(creator, token)` as authoritative (mirror fallback only when no RPC), backing `GET /v1/creators/:address/claimable/:token` (api.md section 3.4b).

## 4. Candle pipeline — venue-continuous (section 5.2, section 8)

### 4.1 Intervals

**Ratified set (D-17):** `1s, 15s, 1m, 5m, 15m, 1h`. Covers every zoom level `lightweight-charts` needs between the spec's endpoints; skips 30s/30m as low-value; 1s is the spec-mandated floor. Trivially adjustable later since candles are derived data.
...
### 4.5 24h change anchor (`change24hPct`, D-40e)

`TokenCard.change24hPct` (api.md section 3.4) needs a 24h-open price anchor. Definition (indexer/API-owned, tunable like the D-22 ranking formulas; frontend renders only):
...
`lastPrice` = `tokens.last_price_eth`. The 1h series already exists (section 4.1); the anchor is a bucket lookup, no new write path. Recomputed alongside `volume_eth_24h` (or resolved at read time from the 1h candles) — never in the WS hot path. Same source used for the `/tokens` list and detail so the value is consistent across surfaces.

> **Anti-drift follow-up (2026-07-10, decisions section 7):** this anchor resolver has **≥2 consumers** (indexer materialization + API `card` projection), so per the anti-drift rule it is hosted **once in `packages/shared`** and both services import it — not duplicated in `apps/indexer/src/change24h.ts` and `apps/api/src/projections/card.ts` (which today returns `change24hPct: null`). Owner: **hoodpad-shared** hosts the resolver; API consumes.

## 5. Confirmation-state pipeline (section 2.1)
...
- Clients upgrade states **locally** from `global:confirmations` watermark messages: any held event with `blockNumber <= safeBlock` becomes `posted_to_l1`, etc. This satisfies "WS update messages" (section 2.1/section 8) with O(1) messages per watermark advance instead of O(rows) — no per-row fanout, no DB reads in the hot path. *Ratified (D-20).*
...
## 6. Metadata integrity pipeline (section 8.3)
...
1. `GET {metadata_uri}` (or `{R2_METADATA_BASE_URL}/{metadataHash}.json` if the event carries no URI — OI-1) with timeout (10s), size cap (64KB), content-type sanity.
   - **Split-horizon fetch base (section 8.3, 2026-07-12 fix).** The stored `metadata_uri` is the **browser-visible** object URL (public CDN base, or in the minio-backed compose stacks the host-mapped `localhost:{MINIO_PORT}` origin). That origin is **unreachable from inside the indexer container** — `localhost` there is the container itself → `ECONNREFUSED` → the row stays `unfetched` and its `image_url` never populates. The verifier therefore fetches from an **internal** base: `METADATA_FETCH_REWRITE_FROM`→`METADATA_FETCH_REWRITE_TO` rewrites the public prefix to the container-internal service DNS (`http://minio:9000/…`) at **fetch time only** — the stored/displayed `metadata_uri` + extracted `image_url` keep the public value (they are canonical, user-facing). This is the same server-only internal-endpoint pattern as the web `API_BASE_URL_INTERNAL` split. All three compose stacks (dev/testnet/mainnet) set it while backed by minio; a real R2/CDN deploy unsets it (the CDN base is reachable everywhere) and points `R2_METADATA_BASE_URL` at the R2 base. Env pair is both-or-neither (fail-closed on a half-config, `apps/indexer/src/config.ts`).
...
**Admin re-verify seam (X-9).** `metadata_verifications` is indexer-owned; the API is read-only on it (it must not `UPDATE` an indexer table). The admin `POST /v1/admin/metadata/:token/reverify` (api.md section 3.6) therefore does **not** write the table — it publishes a control message on the Redis channel `control:reverify` (`{ token }`). The indexer's verifier subscribes to `control:reverify`, marks the row `unfetched` (or bumps its next-attempt), and the poller picks it up. The indexer remains the **sole writer** of `metadata_verifications`; the API only requests.

## 7. Ponder specifics

### 7.1 Idempotency & reorg safety

- All event tables keyed by `(tx_hash, log_index)`; handlers are pure upserts; re-delivery is a no-op (candles guarded by high-water mark, section 4.2).
- Ponder's built-in reorg handling rolls back onchain tables. Offchain tables (`confirmation_watermarks`, `eth_usd_snapshots`, `metadata_verifications`, `moderation_status`) are deliberately outside that mechanism: watermarks are reorg-immune by construction (section 5.3); metadata verifications keyed by token address survive harmlessly (a rolled-back token that re-appears re-seeds the same row).
...
Ponder-managed (schema in `ponder.schema.ts`): `tokens`, `trades`, `transfers`, `graduations`, `fee_collections`, `balances`, `candles`, `creator_claimable` (section 3.12 — Phase-2 creator-fee roll-up, reorg-tracked; empty on v1), and `creator_token_claimable` (section 3.13 — Phase-2 post-grad split roll-up, reorg-tracked; empty on v1). Side-process-managed (plain migrations in `apps/indexer/migrations/`): the four **[offchain]** tables. **There are NO external writes into Ponder-managed tables — none.** VERDICT (2026-07-11, OI-11 verified — see section 10 row + decisions section 11): on the pinned ponder 0.16.8 a direct external `UPDATE` is NOT safe for `tokens` (indexing-store cache full-row flushes silently revert external column updates on rows the handlers keep mutating), and Ponder's docs forbid external writes outright ("Direct SQL queries should not insert, update, or delete rows from Ponder tables"). **REWORKED (2026-07-11): the D-48c sidecar is implemented as pure READ-DERIVATION** — the `confirmation_watermarks` singleton is the sidecar, the per-row `confirmation_state` columns were removed from `ponder.schema.ts`, the tracker (`src/confirmation.ts`/`confirmationStore.ts`) writes only the watermark singleton, and readers derive the tier per row (section 3.8/section 5). A per-row `event_confirmations` join table was weighed and rejected: the tier is fully determined by `(block_number, watermarks)`, so per-row rows would re-introduce O(rows) writes for zero information.

**Offchain migration application (2026-07-12, robbed-indexer — replaces the I-5b compose stopgap; crash-recovery boot fixed 2026-07-15).** The plain migrations are phased in `src/offchainMigrations.ts` (single implementation, consumed by `scripts/migrate.ts` and the sidecar boot): **phase 1** (offchain tables → `public`) applies any time; **phase 2** (0003 pg_trgm GIN indexes + 0005 flow views + 0007 address_pnl views, which reference Ponder-managed tables) is applied **at sidecar boot** (`startSidecars`) on **every** indexer process start. Compose sets `INDEXER_SIDECAR_BOOT=eager` because `ponder start` crash recovery resumes from `_ponder_checkpoint` and does not replay `:setup`; the `:setup` handler remains an idempotent fallback. Sidecar boot waits/retries until Ponder's `database.migrate()` has built its tables, so there is still no `/ready` polling; Ponder drops tables **with CASCADE** on a dev schema rebuild, silently dropping external views, so one-shot application is insufficient — every-boot re-application (all statements idempotent, test-enforced) is required. Compose therefore runs `migrate` only pre-start (phase 1 + assertions); the previous `/ready`-gated re-run loop is gone. Fresh-DB ordering (PORT-1 root cause) is covered by `test/offchainMigrations.test.ts`.

### 7.4 Dynamic sources

- V3 pools: Ponder `factory()` source over `Graduated(pool)` — only graduated pools are indexed.
- LaunchToken `Transfer`: `factory()` source over `TokenCreated(token)` (ratified D-16).
...
2. **Runnable spike (no Docker: anvil + `ponder dev` on its embedded PGlite — no Postgres daemon needed).** Minimal `SpikeFactory.createChildAndTrade()` emits `ChildCreated` (factory registration event) then calls the fresh child, which emits `Trade` — same tx, logIndex 0/1. Result: all four events captured, in `(block, logIndex)` order, with **zero** handler errors, on **both** paths:
   ...
   **Ordering guarantee the handlers may rely on (and one contract-side assumption to keep true):** capture is block-granular, and Ponder delivers same-block events to handlers in `logIndex` order — so the `TokenCreated` handler runs before the same-tx `Trade` handler **iff `TokenCreated`'s logIndex is lower**, which the D-15 emission order guarantees (factory emits `TokenCreated` before the initial buy executes on the curve). If M1 artifacts ever inverted that emission order, the `Trade` handler would fire before the token row exists — that would be an event-shape/ordering divergence to escalate per section 3, not to work around. M2-5 handlers should keep the cheap guard of upserting/asserting token existence in the `Trade` handler.

## 8. Redis pub/sub → Bun WS fanout (section 8)

The WS server is a small Bun process (lives in `apps/api` deployment or standalone — see api.md section 7; the *contract* is defined here). Channel names and message schemas live in `packages/shared` (`channels.ts`, `ws-messages.ts`) — single source for indexer (publisher), WS server (relay), frontend (consumer).

### 8.1 Channel taxonomy

| Redis channel | Content | Consumers |
|---|---|---|
| `global:launches` | new `TokenCreated`, `graduated` announcements | Discover ticker (section 5.1) |
| `global:trades` | every trade (curve + v3), throttle-ready | Discover activity |
| `global:metrics` | coalesced per-token live aggregate snapshots (`token_metrics`, D-70) | Discover card grid live-patch (web.md section 3.1) |
| `global:confirmations` | watermark advances + reorg notices | all pages (badge upgrades) |
| `token:{address}:trades` | trades for one token | Token Detail feed (section 5.2) |
| `token:{address}:candles:{interval}` | candle upsert per trade per interval | chart live updates |
| `token:{address}:events` | `graduated`, `metadata_verified`, `fee_collected`, `creator_fee_split`, `creator_fee_claimed`, state changes | safety strip (D-57), venue switch, creator claim surface |
...
- `type:'fee_collected'` data: `{ token, pool, lpTokenId, amountToken, amountWeth, blockNumber, txHash, logIndex, blockTimestamp, confirmationState }` — published on `token:{address}:events` from the V3 `Collect` handler (section 3.5), feeds the treasury fee-accrual dashboard. **(X-6 — was promised in the section 8.1 channel taxonomy but missing from this union; now defined. hoodpad-shared must add the matching schema to `packages/shared/ws-messages.ts`.)**
- `type:'creator_fee_split'` data: `{ token, creator, creatorAmountToken, creatorAmountWeth, treasuryAmountToken, treasuryAmountWeth, blockNumber, blockTimestamp, txHash, logIndex, confirmationState }` (D-69) — published on `token:{address}:events` from the `LPFeeVault:FeesSplit` handler (section 3.13); the 50/50 split of a graduated pool's fees, per launch token. Schema `wsCreatorFeeSplitDataSchema` in `packages/shared/ws-messages.ts`.
- `type:'creator_fee_claimed'` data: `{ creator, token, amount, blockNumber, blockTimestamp, txHash, logIndex, confirmationState }` (D-69) — published on `token:{address}:events` from the `CreatorVault:CreatorTokenClaimed` handler (section 3.13); a creator pulled an accrued post-grad ERC20 balance. Schema `wsCreatorFeeClaimedDataSchema`. (Channel decision: see section 8.1.)
- `type:'token_metrics'` data: `{ token, priceEth, mcapEth, volume24h, change24hPct, progressPct, status, graduated, blockNumber, ts }` (D-70) — published on `global:metrics`, **COALESCED** per token (≤1 / `WS_METRICS_THROTTLE_MS` ≈ 2s, trailing-edge, last-write-wins by `blockNumber`) from the token-metrics coalescer on each indexed curve `Trade` / V3 `Swap` / `Graduated`. ETH-first aggregate snapshot; every field reuses a `tokenCardSchema` scalar so the push payload and the REST card projection can't drift. Schema `wsTokenMetricsDataSchema` in `packages/shared/ws-messages.ts`. Publish point + throttle design: section 8.4a.

Amounts serialize as decimal strings (uint256 > JS safe integer).

### 8.3 Latency budget (<500ms event-to-browser, section 8)
...
WS provides *freshness*, REST provides *truth*. On reconnect (or detected `seq` gap), the client re-subscribes and calls the REST backfill endpoints (`GET /v1/tokens/:addr/trades?since=`, `/candles?from=` — api.md) to heal, then resumes streaming. The WS server keeps **no replay buffer** in v1 (ratified D-23: replay buffers add state to the fanout tier for a problem REST already solves). Heartbeat ping/pong every 15s; dead sockets reaped.

### 8.4a `token_metrics` coalescer (D-70)

Discover cards render DERIVED aggregates (`mcapEth`, `progressPct`, `status`, `change24hPct`, `volume24h`) that the raw per-event `trade`/`candle`/`graduated` messages don't carry — `change24hPct` in particular needs a 24h-open ANCHOR (the most-recent 1h candle at/before now−24h, or the first-trade price) the handler doesn't hold. Before D-70 a post-graduation V3 `Swap` therefore left a card's mcap/price stale on the grid ("swap didn't update the discovery data"). This coalescer closes that gap; the frontend subscribes `global:metrics` and patches its cached `tokens` list / tape registry by reference (no refetch, no client math — web.md section 3.1).

- **Channel:** `global:metrics` (`GLOBAL_METRICS`, now in `GLOBAL_CHANNELS`; the Bun WS relay explicit-SUBSCRIBEs it via the `GLOBAL_CHANNELS` loop, like the other globals). Message `token_metrics` (section 8.2), payload `wsTokenMetricsDataSchema`.
- **Publish points:** every indexed curve `Trade` (`src/handlers/trade.ts`), V3 `Swap` (`src/handlers/swap.ts`), and `Graduated` (`src/handlers/graduated.ts`) calls `enqueueTokenMetrics({ token, blockNumber, blockTimestamp })` — a pure in-memory push, **NO DB read on the handler hot path** (publish.ts's zero-DB invariant is untouched; the raw trade/candle publishes are unchanged).
- **Throttle / coalescing (`src/tokenMetrics.ts` + `src/tokenMetricsStore.ts`):** a single fixed-interval flush (`WS_METRICS_THROTTLE_MS`, default 2000ms; env-overridable) drains the pending map and does ONE BATCHED 24h-anchor read for the whole batch — the SAME anchor inputs the API card projection uses (`getChange24hAnchors` shape: earliest-trade price + the 1h candle at/before the cutoff) through the SAME shared `computeChange24hPct` resolver, so a live-patched card equals a REST refetch. Per-token last-write-wins by `blockNumber` (an out-of-order straggler never downgrades a token's pending sample); each token publishes ≤1 / window (the trailing edge). **Decision (own it):** chosen over a per-token `setTimeout` so the anchor read is bounded to ≤1 batched query per window *total* — the strongest reading of the "bounded by the throttle, not recomputed per trade" requirement (api.md section 5). This is a periodic derive like the `volume_eth_24h` decay / flow / pnl jobs — NOT on the WS hot path.
- **Gating:** started from the sidecar (`startSidecars`) because it needs the throttled anchor read; `enqueue` is backfill-suppressed (no-ops while the publish gate is closed) and unarmed with `INDEXER_SIDECARS=off` (handler enqueues no-op → no memory growth, no publish). A failed anchor read is dropped + logged (advisory freshness; REST heals and the next trade re-triggers).
- **Anti-drift flag (routed to robbed-shared / robbed-architect):** the `mcapEth` / `progressPct` / `status` derivation now has ≥2 consumers (API card projection + this indexer publish) and is currently MIRRORED in `src/tokenMetrics.ts` because `packages/shared` was frozen for D-70. Hosting it once in `packages/shared` (alongside the already-shared `computeChange24hPct`) is the anti-drift follow-up. `change24hPct` is already the one shared resolver, so it can never drift.

## 8.5 Bot/farm detection heuristics (section 8.5, v1.2)

**M2 feature.** On a chain where >50% of flow is programmatic (section 2.2), the indexer labels bot/farm activity so the Top Holders table + safety strip (section 5.2, D-57–58) and an internal flow-quality dashboard can show **how much of a token's flow and holder set is organic**. Implemented as **SQL views + scheduled jobs over the existing `trades` and `transfers` tables** (section 3.2/section 3.6) — **no new event families, no hot-path cost** (these run as periodic jobs, like the `volume_eth_24h` decay job, section 4.4). **Strictly advisory: labeling only — it never gates any chain interaction, listing, or trade** (consistent with section 8.4 / section 8.4; moderation gates listing, this labels flow). Advisory, tunable-with-data, and always presented as estimates/ranges (section 5.2 forbids false precision).
...
- **Tables (offchain, indexer-owned — like the other side-process tables section 3.11):** `address_flags(address, flags[], cluster_id, updated_at)` and `token_flow_stats(token_address, organic_holder_pct_low, organic_holder_pct_high, organic_volume_pct, flagged_cluster_vol_pct_24h, updated_at)`. Ranges (`_low`/`_high`) because the heuristics are estimates.
- **Organic-holder %** = share of a token's holders (from `balances`, section 3.6) NOT carrying a bot flag; **organic-volume %** = share of curve volume from unflagged addresses; **wash-flagged volume is excluded from organic volume** (heuristic 4).
- **Consumers:** API exposes these on the token-detail `trust` payload (api.md) and an internal `/v1/admin`/dashboard endpoint; the frontend renders them (web.md Top Holders table + safety strip, D-57–58). The **gate-7 cluster-alert metric** (`funding_cluster_vol_share` vs the M0 thresholds, section 10) reads `token_flow_stats`.
- **Never** written into the Ponder reorg-tracked store's trade/balance columns; these are derived side tables, rebuildable from `trades`+`transfers` (section 4.4 rebuild extends to them).

### 8.5.3 hood.fun traction snapshot (section 3/the open items/Gate G-A)

A weekly scheduled job records a **source+timestamped** snapshot of hood.fun traction (tokens created/day, graduation count, visible volume) via own indexer (if hood.fun contracts are indexable) or a Dune query — **never a hardcoded metric (section 2)**. Stored in `competitor_snapshots(source, captured_at, tokens_per_day, graduations, visible_volume_eth)`; feeds **Gate G-A.2** (Gate G-A). Manual/Dune until the job lands.

## 8.6 Portfolio `address_pnl` roll-up (section 5.4)

**M2 feature (Phase-2 page surfaced day 1 by the ROBBED_ redesign).** A per-**address** portfolio roll-up backing `GET /v1/portfolio/:address` (api.md section 3.4a; db-rows.ts `AddressPnlRow`). Same shape as the section 8.5 side jobs: **offchain, indexer-owned table + SQL views + a scheduled recompute-from-raw job** over the Ponder `trades`+`transfers`+`tokens` tables — advisory / read-only, never gates chain state or listing (section 8.4). Fully rebuildable from raw events (section 4.4): the job TRUNCATEs and re-inserts the whole set each tick, so the recompute **is** the rebuild path (no incremental writer to drift from).

- **Table (offchain, `public`, migration `0006_address_pnl.sql`):** `address_pnl(address, first_seen_at, last_active_at, trade_count, tokens_created, total_eth_in, total_eth_out, realized_pnl_low, realized_pnl_high, pnl_confidence, updated_at)`. Aggregate across ALL of an address's tokens; the per-(token, holder) detail stays in `balances` (section 3.6) — this is its address-level roll-up, NOT a duplicate.
- **Views (Ponder schema, `0007_address_pnl_views.sql`):** `pnl_trade_legs` (per-(address, token) buy/sell ETH+token totals, split by venue, `has_v3`), `pnl_address_activity` (trade count + first/last trade), `pnl_address_seen` (first/last Transfer touch; zero address excluded), `pnl_tokens_created` (creator counts, section 7).
- **Compute (pure, `src/pnl/compute.ts`, unit-tested):** average-cost **realized** PnL over the closed (matched) leg per token, summed per address. **Realized is a RANGE** because V3-leg cost basis is best-effort (V3 `trader`/cost-basis keys on the tx-sender EOA `event.transaction.from` — exact for direct swaps, best-effort for contract-mediated / aggregator swaps where `tx.from` is a relayer — **D-75**, supersedes the recipient-is-router basis of D-16/OI-5): the band brackets **curve-only realized (V3 attribution discarded) vs full realized (V3 trusted)** → `low = min`, `high = max`. Curve-only address ⇒ `low == high`, `exact`; any V3 leg ⇒ `estimated`; no cost basis anywhere ⇒ `pnl_confidence = null` (range 0) so the API surfaces `pnlAllTime = null` (section 5.2 forbids false precision).
...
- **Cadence:** wall-clock `setInterval` (default 60s, `PNL_JOB_INTERVAL_MS`), same pattern as the section 8.5 flow job; wired from the sidecar boot (`src/pnl/job.ts` + `src/pnl/store.ts`).
...
- **Bot/farm heuristics (section 8.5, v1.2):** funder-cluster fixture (20+ micro-funded wallets from one funder → all `farm`, one `cluster_id`); sniper timing boundary (buy at t+59s flagged, t+61s not); **own-Router trade is NOT flagged `programmatic`** (whitelist); wash-loop round-trip excluded from organic volume; same-second ≥3-pool exit → `arb/exit`; `organic_holder_pct` computed as a range; all labels advisory (no code path gates a trade/listing on a flag).

### 9.2 Integration

Ponder test harness (or anvil fork per section 10 gate 3 fixtures once M1 lands) replaying a canned event log through real handlers into ephemeral Postgres; asserts every table, Redis messages captured via a test subscriber.
...
- Derived rebuild: section 4.4 script; runbook entry: safe to run while live indexing is paused.
- Ponder crash/restart: resumes from checkpoint; upsert idempotency covers the overlap window.

### 9.4 Monitoring (gate 7, section 10)
...
- `metadata_unfetched_total`, `metadata_mismatch_total` — mismatch > 0 pages review (the safety strip shows it either way, D-57).
- **Invariant metrics (gate 7 explicitly):** per-token `real_eth_reserves` vs on-chain balance spot-check sampler; graduation single-fire violation (second `Graduated` for a token ⇒ page immediately); `fee_collections.recipient != treasury` ⇒ page immediately; trade with `fee_eth > 2%` of leg ⇒ page (fee ceiling section 6.4).
- `redis_publish_errors_total`, `ws_connected_clients`, `eth_usd_snapshot_age_seconds` (alert > 5m — USD displays go "dated", never stale-silent, section 2).

## 10. Open items & decisions needed

| ID | Item | Recommendation | Status |
|---|---|---|---|
| OI-1 | Exact ABI shapes for `TokenCreated`/`Trade`/`Graduated` | `Trade` emits post-trade reserves + fee; `TokenCreated` carries `metadataUri` | **RESOLVED — D-15 (2026-07-09).** Canonical shapes in contracts.md section 2, mirrored in section 3 above |
| OI-2 | `Trade.ethAmount` semantics: gross (incl. fee) vs net | — | **RESOLVED — D-15:** `ethAmount` is **gross**, `fee` is a separate field; net = gross − fee |
| OI-3 | Indexing LaunchToken `Transfer` (6th event family) for exact balances | — | **RESOLVED — D-16:** adopted; sole source of balance truth |
| OI-4 | Pre-graduation V3 pool activity: index or ignore? | — | **RESOLVED — D-16:** not indexed; curve is sole venue until `Graduated`; gate-7 alerting covers pool griefing |
| OI-5 | V3 `Swap.recipient` is often a router, so per-EOA `total_eth_in/out` for v3 legs is approximate | — | **RESOLVED — D-16** (2026-07-09); **SUPERSEDED — D-75 (2026-07-14):** `trader` + best-effort v3 cost-basis `holder` now derive from `event.transaction.from` (the tx-sender EOA) — exact for direct SwapRouter02 swaps, best-effort only for contract-mediated / aggregator swaps until Phase-2 4337; balances stay exact via `Transfer`. Requires a reindex (handler `build_id` change) |
| OI-6 | ETH/USD source on 4663 (Chainlink feed existence unverified) | Chainlink if deployed on 4663, else DefiLlama with timestamp; config-driven | **VERIFIED 2026-07-11 (decisions section 11): Chainlink ETH/USD feed EXISTS on 4663** — proxy `0x78F3556b67E17Df817D51Ef5a990cDaF09E8d3A9` (aggregator `0x6091E64eb7138EEF066a80FD3A0d7427B91f2721`, 8 decimals, `description()` = "ETH / USD", confirmed via `eth_call` on the public 4663 RPC). Poller (M2) reads the feed, source label `chainlink:4663` (section 3.9), staleness-checked; DefiLlama/Coinbase HTTP stays the configured fallback + the LOCAL/TESTNET source. Address recorded in spec **D-51**, the open items OI-6 closed. **Poller IMPLEMENTED 2026-07-11** with the D-51 fail-closed assertions — see section 3.9 status |
| OI-7 | Candle interval set `1s/15s/1m/5m/15m/1h` | — | **RESOLVED — D-17:** as proposed |
| OI-8 | `safe`/`finalized` block-tag support on Robinhood RPC | Verify day 1 of M2; fallback = L1 rollup-contract watermarks | **VERIFIED 2026-07-11 (decisions section 11): both tags SUPPORTED** on `https://rpc.mainnet.chain.robinhood.com` — `eth_getBlockByNumber["safe"/"finalized",false]` return full Arbitrum block objects (incl. `l1BlockNumber`); observed safe 7,082,802 > finalized 7,078,967, `eth_chainId` = 4663. section 5.1 tag path is live; M2-3b L1-watermark reader NOT funded — dormant pre-sanctioned fallback only |
| OI-9 | Confirmation propagation via watermark broadcast (O(1)) vs per-event WS updates | — | **RESOLVED — D-20:** watermark broadcast |
| OI-10 | Server-side verification of `imageHash` inside metadata JSON | — | **RESOLVED — D-23:** deferred post-v1; carried in JSON, client-verifiable |
| OI-11 | External `UPDATE` of `confirmation_state` on Ponder-managed tables vs sidecar table | Direct update if the pinned Ponder version tolerates it; else sidecar `event_confirmations` join | **VERIFIED 2026-07-11 (decisions section 11): direct UPDATE NOT tolerated by ponder 0.16.8 for hot rows** — the indexing-store cache retains rows in memory across realtime blocks (cached keys are never re-read from the DB) and flushes ALL columns from the cached copy, so an external `confirmation_state` upgrade on `tokens` (mutated on every Trade) is silently reverted by that token's next flush → section 5.1 monotonicity violated. **D-48c sidecar is REQUIRED**. **Rework LANDED 2026-07-11 (robbed-indexer):** implemented as pure read-derivation — per-row columns removed from `ponder.schema.ts`, tracker writes only the `confirmation_watermarks` sidecar singleton, tiers derived at read time (section 3.8/section 5/section 7.3) |
| OI-12 | WS replay buffer vs REST-heal on reconnect | — | **RESOLVED — D-23:** REST-heal only in v1 |
| OI-13 | V3 Factory / NPM addresses on 4663 | From official registry at implementation time; startup fails if unset (never guessed) | **OPEN — the open items** |

## 11. Definition of done (M2, indexer)

- [ ] Handlers implemented for all six event families (D-15–16), all idempotent on `(tx_hash, log_index)`
- [ ] Schema materialized exactly as section 3 (Ponder tables + offchain migrations), `pg_trgm` GIN indexes present, startup assertions in place
- [ ] `creator` + `creatorFeeBps` on every token row from first indexed event (section 7)
...
- [ ] Confirmation tracker live: watermark sidecar + read-time tier derivation (OI-11/D-48c — no per-row writes), `global:confirmations` broadcast; transitions unit-tested incl. monotonicity and reorg
...
- [ ] **(v1.2)** Bot/farm heuristics (section 8.5) as SQL views/jobs over `trades`+`transfers`; `address_flags` + `token_flow_stats` populated; own-Router whitelisted; wash excluded from organic volume; advisory-only (no chain/listing gate); weekly hood.fun `competitor_snapshots` job; all tested per section 9.1
- [ ] No hardcoded market metrics anywhere; USD only via `eth_usd_snapshots` (section 2); hood.fun/competitor snapshots source+timestamped, never hardcoded
- [ ] `bun test` green; any artifact-vs-D-15 event-shape divergence found at implementation time escalated to hoodpad-architect, not patched around
