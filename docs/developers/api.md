...
**Spec coverage:** section 5.1 (search, discover data), section 5.2 (token detail data), section 5.3 (launch flow: uploads + metadata hash), section 8 (Hono on Bun: R2 presigned uploads, moderation queue, search API; WS fanout), section 8.3 (canonical metadata JSON to R2), section 8.4 (moderation), section 2 (no hardcoded market metrics), section 2.1 (confirmation states in responses).
...
1. **R2 presigned upload flow + canonical metadata publication** for the launch flow (section 5.3, section 8.3) — the only write-heavy path.
2. **Read API** over indexer tables: token lists/sorts/filters, token detail, trades, candles, holders, King of the Hill, treasury fee dashboard (section 5.1, section 5.2, section 6.4).
3. **Search**: one endpoint, `pg_trgm` over name / ticker / contract address / creator address (section 5.1).
4. **Moderation** (section 8.4): upload-time enforcement (MIME sniff, ≤4MB, re-encode), auto-moderation pipeline with pluggable vendors (an open item), impersonation flags, admin queue. Moderation gates **listing visibility only** — no API path may ever mutate or depend on mutating chain state.
5. **WS fanout host**: the Bun WebSocket server (Redis subscriber → socket fanout) ships inside `apps/api` as a separate entrypoint/process (`apps/api/src/ws.ts`), sharing types with the HTTP app but not the request lifecycle. Contract defined in indexer.md section 8.

Non-goals: no chain writes ever (all transactions are wallet-signed client-side, section 5.3 one-tx create); no server-side trading logic; no USD constants (section 2 — USD derived from `eth_usd_snapshots` at response time, always with `asOf` timestamp).
...
- Every event-derived object includes `confirmationState` (section 2.1).
- Every USD-derived field ships as `{ usd: string, ethUsd: string, asOf: string }` — computed at request time from the latest `eth_usd_snapshots` row, never a constant (section 2). If the snapshot is older than 5 minutes, `stale: true` is added.
...
- Server-sorted tables (`/tokens/:address/trades`, `/tokens/:address/holders` — D-59): additionally accept `?sort=<field>&dir=asc|desc`. The `sort` field comes from a **closed allowlist** (robbed-shared `TRADE_SORT_FIELDS` / `HOLDER_SORT_FIELDS`) mapped API-side to a fixed column — a value outside the allowlist (or a `dir` other than `asc|desc`) is `400 invalid_request`. This is the ORDER BY security boundary: no caller string ever reaches SQL. These endpoints return the uniform `{ items, nextCursor }` envelope (shared `paginatedTradesResponseSchema` / `paginatedHoldersResponseSchema`), keyset-paginated over the active `(sort_col, tiebreak)` — never client-side sort/rank (server-authoritative, D-22/D-59).
- Listing-gated endpoints (`/tokens` list, `/search`) exclude `moderation_status.visibility = 'hidden'`; `pending_review` behavior per section 4.5. Direct fetch by address (`/tokens/:address`) returns hidden tokens with `moderation: { visibility }` populated so the frontend can render a "hidden by moderators" state instead of a 404 — hiding is a listing concern, the token exists on-chain regardless (section 8.4). *(Ratified — D-21.)*
- Response DTO types all live in `packages/shared` (section 5 below); the frontend imports them, never redeclares.

## 3. Endpoint inventory

### 3.1 Launch flow — image upload (section 5.3, section 8.4)

**`POST /v1/uploads/image` → presign is deliberately NOT used for the raw client file.** Design decision: the image is uploaded **through** the API (multipart), not via a presigned PUT directly from the browser, because section 8.4 requires MIME sniff + re-encode *before* anything reaches public storage. A presigned direct PUT would put unmoderated bytes on the CDN. The "R2 presigned upload" of section 8 is satisfied on the API→R2 leg: the API streams the re-encoded object to R2 using its own credentials (server-side presign/direct SDK put). *(Ratified — D-19; section 5.3/section 8 amended accordingly.)*

```
POST /v1/uploads/image        multipart/form-data, field "image"
  Limits: ≤ 4MB (section 5.3) enforced pre-buffer via Content-Length + streamed cap
  Pipeline (section 8.4, synchronous):
    ...
    3. sha256 + keccak256 of the RE-ENCODED bytes (the re-encoded object is the canonical image;
       its hash is what goes into metadata JSON as imageHash — section 8.3)
    4. PUT to R2: images/{keccak256}.webp (content-addressed → idempotent, dedupes)
    5. Enqueue auto-moderation job for the image (section 4) keyed by image hash
  200 → { imageUrl, imageHash, width, height, bytes }
  4xx → oversized | unsupported_type | decode_failed
```

### 3.2 Launch flow — metadata canonicalization + hash (section 8.3)
...
POST /v1/metadata
  Body: { name, ticker, description?, links?, imageUrl, imageHash }   -- schema-validated (zod, from packages/shared)
    name ≤ 32 BYTES, ticker ≤ 10 BYTES (UTF-8 byte length, NOT char count — must equal the on-chain
      createToken validation `bytes(name).length ∈ [1,32]` / `bytes(symbol).length ∈ [1,10]`, D-30, contracts.md section 2.2;
      a name that passes here must never revert at createToken), description ≤ 500 (section 5.3), links: {website?,x?,telegram?} URL-validated,
      version = 1 (frozen inside the canonicalized hash preimage, D-31),
    imageUrl must be our CDN origin, imageHash must match an object we produced (existence check on R2 key)
      → on mismatch/not-found: HTTP 400, error.code = 'invalid_request' (D-40b — NOT 409/conflict;
  ...
  Pipeline:
    ...
    4. PUT to R2: metadata/{hash}.json  (content-addressed: URL derived from hash → immutable-by-convention,
       and the indexer's weekly re-verify catches any mutation — indexer.md section 6.2)
    5. Run text moderation + impersonation check (section 4.3/section 4.4) — result recorded, NEVER blocks the response
       (moderation gates listing, not creation — section 8.4; user can always send the tx)
...
GET /v1/search?q=<string>&limit=20
  ...
  Behavior:
    ...
    - Else → similarity search over name and ticker:
      ...
      (ticker boost + volume tiebreak + similarity floor 0.25 — ratified defaults, D-22; tunable config)
    - Excludes hidden listings (section 8.4)
  200 → { results: TokenCard[] }   -- same card projection as /tokens (section 5.1 card fields)
```

Single endpoint covering all four fields (name, ticker, contract, creator) as the DoD requires; GIN trgm indexes defined in indexer.md section 3.1.

### 3.4 Token reads (section 5.1, section 5.2)

```
GET /v1/tokens
  ?sort = trending | newest | mcap | volume24h | progress      (section 5.1 sorts)
  &filter = pregrad | graduated | all                          (section 5.1 filters)
  &cursor=&limit=
  trending = vol24h × e^(−age/24h)  (ratified default, D-22; tunable config)
...
GET /v1/tokens/king-of-the-hill                                 (section 5.1 hero)
  ...
  ORDER BY (real_eth_reserves/graduation_eth) * ln(1+volume_eth_24h) DESC  (ratified default, D-22)
  200 → { token: TokenCard }

GET /v1/tokens/:address                                         (section 5.2 detail + safety strip D-57; formerly the Trust panel)
  200 → TokenDetail: TokenCard fields +
    ...
    reserves: { virtualEth, virtualToken, realEth, realToken }  -- live curve state (safety strip section 5.2, D-57)
    graduation: { thresholdEth, progressPct }
    trust: {                                                    -- feeds the safety strip section 5.2, D-57, all derived, none hardcoded
      ...
      feePolicy: { tradeFeeBps, creatorFeeBps },                -- tradeFeeBps = per-token snapshot from tokens.trade_fee_bps
                                                                ...
                                                                --   NOT the live factory config (D-40d — config governs future curves only);
                                                                --   creatorFeeBps present (0), section 7
      organic: {                                                -- (v1.2) section 5.2/section 8.5; from indexer token_flow_stats; advisory
        ...
        methodology: "heuristic — see section 8.5", updatedAt          -- tooltip source; null while stats not yet computed
      }
    }
...
GET /v1/tokens/:address/trades
  ?since=<ts|blockNumber>                                        -- WS reconnect backfill floor (indexer.md section 8.4)
  &sort = age | side | trader | amount | price                  (D-59 allowlist; DEFAULT age)
  &dir  = asc | desc                                            (DEFAULT desc — newest-first)
  &cursor=&limit=                                               -- keyset cursor + page size (config PAGE_LIMIT_DEFAULT=50)
  Trade feed (section 5.2) + WS reconnect backfill (indexer.md section 8.4). SERVER-side sorted + keyset-paginated
  (D-59; server-authoritative, never client sort). Each row includes venue + confirmationState.
...
GET /v1/trades/:txHash
  Optimistic-UI reconciliation lookup (web.md section 4.1: RPC said success but no WS event yet).
...
GET /v1/tokens/:address/holders
  ?sort = rank | address | label | amount | percent             (D-59 allowlist; DEFAULT rank ≡ balance)
  &dir  = asc | desc                                            (DEFAULT desc — biggest balance first)
  &cursor=&limit=                                               -- keyset cursor + page size (config PAGE_LIMIT_DEFAULT=50)
  Top-holders table (section 5.2/D-58), SERVER-side sorted + keyset-paginated (D-59). First page (default
  ...
  200 → { items: [{ address, balance, pct, rank,                -- uniform {items,nextCursor} (paginatedHoldersResponseSchema)
                    ...
                    botFlags?: ('farm'|'sniper'|'programmatic'|'wash'|'arb_exit')[],   -- (v1.2) advisory, indexer section 8.5 address_flags
                    clusterId?: string }],                                             -- shared gas-funder → grouped on the list (section 5.2)
          nextCursor }
  BREAKING (D-59): the pre-redesign { holders, holderCount } envelope is REPLACED by { items, nextCursor }.
    `holderCount` is no longer returned by this endpoint (paginatedHoldersResponseSchema is { items, nextCursor }).

GET /v1/tokens/:address/fees                                    (section 6.4 treasury fee dashboard)
...
GET /v1/stats
  Global: tokens launched, graduations, 24h volume, treasury fees collected. All computed; USD per section 2 convention.
```

### 3.4a Portfolio (section 5.4 — Phase-2 page surfaced day 1 by the ROBBED_ redesign)

Advisory / read-only: no path mutates or depends on mutating chain state (section 8.4). Any address resolves — an unknown address is an **empty** portfolio, never a 404 (the wallet ETH balance is a live chain read independent of the indexer). Shapes are the frozen `@robbed/shared` DTOs (`portfolioSummarySchema`, `portfolioHoldingSchema`, `tokenRefSchema`, `ethPnlRangeSchema`, `portfolio{Holdings,Activity,Created}ResponseSchema`) — never redeclared. **All ETH-first (section 2):** value/PnL are wei decimal strings, USD mirrors derive at request time; **PnL is a nullable RANGE, no false precision (section 5.2).**

> **Spec deviation (flagged for robbed-architect the decision record disposition):** section 5.4 Portfolio was **Phase-2**; the ROBBED_ redesign (D-50; planning doc retired 2026-07-12, history: git) surfaces it day 1. The DTO schemas were ratified into `packages/shared` and this endpoint set implements them, but the **Phase-2 → day-1 promotion itself** still wants a formal the decision record disposition (mirrors the redesign plan's "4 pages incl. Portfolio overrides section 5" item). Recorded here, not self-resolved. *Architect 2026-07-11: DISPOSITIONED — D-50a records the promotion (Portfolio → v1, read-only, no new tx types, no `collect()` UI). The advisory-read semantics of this section (no WS channel; staleTime + refetch) are additionally RATIFIED with the `PORT-*` catalog addendum (apps/web/e2e/user-flows.md section 3b; ratified 2026-07-11 — ledger retired, history: git).*

```
GET /v1/portfolio/:address                                     (section 5.4 stat cells)
  200 → PortfolioSummary: address, firstSeenAt, tradeCount, tokensCreated,
        ...
        totalValueEth, totalValue{usd,ethUsd,asOf},             -- Σ priceable holdings' liquidation value; USD derived (section 2)
...
GET /v1/portfolio/:address/holdings?cursor=&limit=              (section 5.4 HOLDINGS table)
  ...
  PortfolioHolding: token: TokenRef{address,name,ticker,imageUrl,graduated,status},
        ...
        unrealizedPnl: EthPnlRange | null                       -- null when no cost basis (section 5.2)

GET /v1/portfolio/:address/activity?cursor=&limit=             (section 5.4 ACTIVITY tab)
...
GET /v1/portfolio/:address/created?cursor=&limit=             (section 5.4 CREATED tab, section 7)
  Tokens whose on-chain creator == :address — REUSES the TokenCard projection. Listing-gated (section 8.4). Cursor: (created_at, address) DESC.
  200 → { tokens: TokenCard[], nextCursor }
```

**Materialization (indexer `address_pnl`, db-rows.ts `AddressPnlRow`).** The per-address roll-up (`first_seen_at`/`last_active_at`/`trade_count`/`tokens_created`/`total_eth_in`/`total_eth_out`/realized-PnL range `realized_pnl_low/high` + `pnl_confidence`) is a **scheduled recompute-from-raw job** over `trades`+`transfers`+`tokens` (indexer.md section 8.6 / SQL views `pnl_*` + TRUNCATE+re-insert, rebuildable section 4.4). Balances stay Transfer-truth (existing `BalanceRow`, X-4/X-5). **Wallet ETH** = RPC balance read at the API layer; **all-time/unrealized PnL** is computed at request time (live price × balance − remaining basis), NOT materialized.
...
GET /v1/creators/:address/claimable/:token                     (D-69 post-grad 50/50 split, per ERC20)
  200 → CreatorTokenClaimable: creator, token, vault,
        ...
        claimableUsd: UsdValue | null,                         -- WETH leg only (ETH-priced section 2); null for launch-token legs
...
```
GET /v1/confirmations          → { safeBlock, finalizedBlock, latestBlock, updatedAt }   (section 2.1; SSR initial state)
GET /v1/eth-usd                → { price, source, asOf }                                  (section 2; frontend display source)
...
GET /v1/readyz                 → readiness probe, DB+Redis+R2 (gate-7 probes):
  ...
  503 → the STANDARD section 2 error envelope: { data: null, error: { code: 'upstream_unavailable',
...
### 3.5a OG share cards (section 5.2 share card)
...
GET /v1/og/{address}.png        → image/png 1200×630 — the token's ROBBED_ terminal share card
  ...
  Data: reuses the /tokens card projection (name/ticker/status/graduated/progress) + the mini candle
    window (12h @ 15m) + mcap ETH-first with USD `asOf` (section 2 — no hardcoded metric). Token logo is
  ...
  404 → unknown token; 400 → malformed address. Never mutates chain state (section 8.4).
```

### 3.6 Moderation — admin endpoints (section 8.4)

All under `/v1/admin/*`, auth per section 6. Every action audit-logged.
...
POST  /v1/admin/moderation/:tokenAddress/visibility   { visibility: visible|hidden, reason }
        → sets listing visibility ONLY (section 8.4: "admin can hide listings only") — there is deliberately
...
POST  /v1/admin/metadata/:tokenAddress/reverify        → publishes `control:reverify {token}` on Redis; the INDEXER
                                                          ...
                                                          writes indexer-owned tables; indexer.md section 6.2 is the sole writer)
GET   /v1/admin/audit-log?cursor=
```

### 3.7 Internal dashboard endpoints (D-4 — D-54; M2-13/M2-14; Gate G-A.1/G-A.2)
...
same `requireAdmin` cookie session as section 3.6/section 6.2). Chosen over internal-network-only gating per
...
(section 6.3). GET-only → no CSRF (CSRF guards mutations only). **Advisory-only framing is binding
(section 8.4/section 8.5):** everything here is labeling/telemetry — it never gates chain state, listing, or
any user path.
...
GET /internal/flow/:address                                    (M2-13; Gate G-A.1 flow quality)
  ...
  { token,                                                     -- lowercased token address
    organic: OrganicFlow | null,                               -- EXACTLY the shared organicFlowSchema object the Trust
                                                               --   panel gets (section 3.4) — same projection (projections/
                                                               --   trust.ts buildOrganic); null until the section 8.5 job has
                                                               --   computed token_flow_stats. holderPctLow/High is a
                                                               --   RANGE (section 5.2 — no false precision); volumePct and
                                                               --   flaggedClusterVolPct24h are the section 8.5.2 estimates
                                                               --   (wash-excluded organic volume / cluster share of
                                                               --   24h curve volume).
    flagged: {                                                 -- section 8.5 advisory summary over THIS token's current holders
...
GET /internal/competitor-snapshots?cursor=&limit=              (M2-14; Gate G-A.2 traction input)
  200 → { snapshots: CompetitorSnapshotRow[], nextCursor }     -- newest first: ORDER BY captured_at DESC, source DESC;
                                                               --   keyset cursor (captured_at, source), section 2 pagination
  ...
  section 2 discipline: `source` + `captured_at` are NOT NULL by table constraint and always present —
  the endpoint only reads, it can never fabricate a metric. While the snapshot source is
  unconfigured (indexer `unconfiguredCompetitorSource`, the open items / section 8.5.3 "manual until
...
per the D-40c single-consumer precedent they may stay API-local — or robbed-shared may ratify
...
## 4. Moderation pipeline (section 8.4)

### 4.1 Principles

Moderation gates **listing** (visibility in `/tokens`, `/search`; WS `global:launches` is unaffected in v1 — ratified, D-21), never chain state, never creation, never trading. Failure of any vendor **fails open to `pending_review`** visibility policy (section 4.5), never blocks the launch tx.

### 4.2 Upload-time (synchronous, section 3.1)
...
### 4.3 Auto-moderation (async, vendor-pluggable — an open item)

Vendor undecided (the open items), so the pipeline is built against interfaces **defined in `apps/api/src/moderation/vendors/`** (NOT `packages/shared` — D-40c: these interfaces are consumed by exactly one service, the API moderation worker, so they fail the anti-drift ≥2-consumers test and stay API-local), with implementations in the same directory:
...
Jobs run from a Redis-backed queue (BullMQ-class or minimal Redis list worker — implementation choice, OI-A4) after image upload and again keyed to the token at `TokenCreated` (linking image→token). Results land in `moderation_status` (table owned by API; shape in indexer.md section 3.11):

- `csam_flag = true` → `visibility = 'hidden'` immediately + irreversible-by-UI (unhide requires out-of-band process), vendor-mandated reporting hook (NCMEC-class) stubbed behind the vendor interface — legal flow is part of vendor selection (the open items).
...
### 4.4 Impersonation flags (section 8.4)

Static curated list in config/DB (`impersonation_watchlist`: ticker, name variants) covering top-asset tickers (BTC, ETH, top-100 by mcap — list is data, refreshed manually, **never hardcoded metrics**, section 2) and Robinhood **Stock Tokens** tickers (HOOD, AAPL, TSLA-class — source: Robinhood's published Stock Token list at implementation time). At `POST /v1/metadata` and at `TokenCreated`: case-insensitive exact + confusable-normalized (homoglyph fold) match on ticker and name → `impersonation_flag = true`. Flag ≠ hidden: it renders as a warning badge (frontend concern) and pushes the item into the review queue. Admin can clear or escalate to hidden.

**`TokenCreated` observation seam (X-10).** The API has a read-only DB role on indexer tables and no chain subscription, so it cannot itself watch `TokenCreated`. The seam is Redis: the indexer already publishes `launch` on `global:launches` for every `TokenCreated` (indexer.md section 8.1). The API runs a small **moderation worker** (part of the WS process or a sibling worker) that subscribes to `global:launches`, and on each launch performs the token-time checks — impersonation match on the on-chain name/ticker and linking the pre-scanned image moderation result to the token — writing verdicts to `moderation_status` (an API-owned table, section 3.11). No chain read, no indexer-table write; the worker consumes the indexer's existing publish.

### 4.5 `pending_review` visibility policy

`pending_review` items **remain listed** with default trust treatment in v1 (capped beta, low volume, human review SLA is hours) — hiding-by-default would let a vendor outage blank the site. *(Ratified — D-21.)*
...
|---|---|---|
| `metadata.ts` | `TokenMetadata` zod schema (name/ticker/description/links/imageUrl/imageHash/version), `canonicalizeMetadata(obj): Uint8Array`, `metadataHash(obj): Hex` (keccak256 wrapper) + golden fixtures | api (canonicalize+hash), web (pre-sign verify), indexer (verification section 8.3) |
...
| `channels.ts` | channel-name builders (`tokenTrades(addr)`, `GLOBAL_LAUNCHES`, …) — taxonomy in indexer.md section 8.1 | indexer (publish), api/ws (fanout), web (subscribe) |
| `ws-messages.ts` | envelope + per-type message schemas (indexer.md section 8.2), zod-validated | indexer, ws, web |
| `api-types.ts` | `TokenCard`, `TokenDetail`, `TradeRow`, `Candle`, `HolderRow`, `SearchResult`, `UsdValue`, `ConfirmationsResponse`, `EthUsdResponse`, `ModerationQueueItem` (D-40a), error codes, pagination envelope | api (response shaping), web (consumption) |
...
| `db-rows.ts` | row types matching indexer.md section 3 tables | indexer, api |
| `constants.ts` | chain id 4663, WETH address, interval list, size caps, the exact LP copy string | all three |

Canonicalization is defined once, here, RFC-8785-style (UTF-8, sorted keys at every depth, no whitespace, canonical number/string forms); the indexer doc's byte-identical requirement (indexer.md section 6.1) is satisfied by construction because there is exactly one implementation.

**Frozen-schema notes for robbed-shared (this ratification pass):**
- **`api-types.ts` additions (D-40a):** `export type` aliases `ConfirmationsResponse` (`GET /v1/confirmations` → `{ safeBlock, finalizedBlock, latestBlock, updatedAt }`), `EthUsdResponse` (`GET /v1/eth-usd` → `{ price, source, asOf }`), `ModerationQueueItem` (admin queue row, section 3.6) — for naming consistency with the other DTOs.
...
- `allowMethods: GET, POST, OPTIONS`; `allowHeaders: Content-Type` (metadata POST is
  ...
  `exposeHeaders: Retry-After` (the browser client's 429 backoff, section 6.3, needs to read it);
...
- **Scoping decision:** `/v1/admin/*` and `/internal/*` are the cookie+CSRF SIWE surface (section 6.2) and
...
- **Stored-link XSS** (threat-model UM-5) — `links` are validated with an **`https:`-only scheme allowlist** (reject `javascript:`, `data:`, `http:`, and any non-`https` scheme) at `POST /v1/metadata`; the frontend additionally renders them with `rel="noopener noreferrer"` under CSP (web.md section 5). Syntactically-valid-but-hostile schemes never pass.
...
- **Moderation bypass via metadata mutation** — R2 objects content-addressed by hash; a changed object breaks the hash and the indexer's re-verify flips the Trust verdict (indexer.md section 6.2); moderation keys off the bytes we scanned.
- **Admin endpoints** — allowlist + SIWE + audit log; no chain-affecting capability exists to escalate to (section 8.4 by construction).
...
Two processes from one codebase: `apps/api/src/index.ts` (Hono HTTP) and `apps/api/src/ws.ts` (Bun WS fanout, indexer.md section 8). Both Bun; share `packages/shared` and config loader; scale independently (WS is fan-out-bound, HTTP is DB-bound). Postgres access: read-only role for indexer-owned tables (enforces "indexer owns writes"), read-write role limited to `moderation_status`, `moderation audit log`, `impersonation_watchlist`.
...
|---|---|---|---|
| OI-A1 | "R2 presigned uploads" (section 8) read as browser-direct presign vs API-mediated upload | — | **RESOLVED — D-19 (2026-07-09):** API-mediated; presign only on the API→R2 leg; section 5.3/section 8 amended |
| OI-A2 | Search ranking (ticker boost ×1.2, volume tiebreak, similarity floor 0.25) | — | **RESOLVED — D-22:** as stated; tunable config, tune in beta |
| OI-A3 | `trending` sort + King-of-the-Hill formula | — | **RESOLVED — D-22:** KotH `progress × ln(1+vol24h)`; trending `vol24h × exp(−age/24h)`; tunable config, not consensus values |
...
| OI-A5 | `pending_review` default visibility + whether WS `global:launches` respects moderation | — | **RESOLVED — D-21:** default-listed; WS ticker unmoderated in v1 (moderation-aware fanout would need hot-path DB reads) |
| OI-A6 | Hidden tokens on direct `/tokens/:address` fetch: 404 vs visible-with-flag | — | **RESOLVED — D-21:** visible-with-flag |
| OI-A7 | Moderation vendor selection (CSAM hash-match + NSFW classifier) + mandated-reporting legal flow | Interfaces (section 4.3) let M2 ship with stubs behind a boot guard | **OPEN — the open items** |
| OI-A8 | Admin allowlist membership (ties to Safe signer set, the open items) | Config-driven allowlist; populate when signer set decided | **OPEN — the open items** |
| OI-A9 | Stock Token / top-asset ticker list source + refresh cadence for impersonation watchlist | — | **RESOLVED — D-23:** curated, source-cited, dated data file, refreshed ≥ monthly |
| OI-A10 | ETH/USD endpoint source (shared with indexer OI-6) | Single source of truth = `eth_usd_snapshots` table (indexer poller writes, API reads) | Aligned; underlying source **OPEN — the open items** (indexer OI-6) |

## 10. Definition of done (M2, API)

- [ ] All endpoints of section 3 implemented with request/response schemas from `packages/shared` (zod-validated in, typed out); frontend imports, never redeclares
...
- [ ] All section 5.1/section 5.2 read endpoints live: lists with all five sorts + three filters, KotH, detail with full safety-strip payload (D-57; exact LP copy string from `constants.ts`), trades (with `since` backfill), candles (all six intervals), holders with flags, fees dashboard, confirmations, eth-usd
...
- [ ] `confirmationState` present on every event-derived response object (section 2.1)
- [ ] No USD/market constant anywhere; every USD field carries `asOf` (section 2)
