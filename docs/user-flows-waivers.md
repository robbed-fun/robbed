...
| `COLLECT-1` | on-chain · indexed | UI | `collect(tokenId)` is permissionless and treasury-facing; it has **no surface in the three v1 pages** (Portfolio/treasury tooling is out of scope, §5.4). Asserted on-chain (fees route to the fixed treasury, principal stays locked) + indexed (collect event), never UI. |
