...
| **EXECUTION** | What to build and when — checkbox state and task detail. | [`implementation-plan.md`](implementation-plan.md) (the single `/goal` authority), [`plans/*-plan.md`](plans/) (per-service detail), [`goal-completion-plan.md`](goal-completion-plan.md) (goal-scoped sequencing, subordinate to the master plan) |
