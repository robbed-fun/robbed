...
contract DeployHarness is Deploy {
    function selectMode(uint256 chainId) external pure returns (Mode) {
        return _selectMode(chainId);
    }

    /// @dev Resolves mode from the (cheatcode-set) chain id, then returns the default constants path.
...
contract DeployModesTest is Test {
    ...
    function test_selectMode_threeWaySplit() public view {
        assertEq(uint256(harness.selectMode(4663)), uint256(Deploy.Mode.Live), "4663 -> live");
        assertEq(uint256(harness.selectMode(46_630)), uint256(Deploy.Mode.Testnet), "46630 -> testnet");
        assertEq(uint256(harness.selectMode(31_337)), uint256(Deploy.Mode.Local), "31337 -> local");
        assertEq(uint256(harness.selectMode(1)), uint256(Deploy.Mode.Local), "1 -> local");
        ...
        assertEq(uint256(harness.selectMode(46_646)), uint256(Deploy.Mode.Local), "46646 stays non-testnet");
    }
