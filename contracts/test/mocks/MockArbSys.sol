...
/// @notice Unit/fuzz/invariant tests etch this at `address(100)` (contracts.md):
...
/// contracts.md) on a local Foundry chain `block.number` is a real L2-style counter
...
contract MockArbSys is IArbSys {
    ...
    function arbBlockNumber() external view returns (uint256) {
        // block.number tolerated ONLY here — mock harness (contracts.md).
