...
/// can never touch live-curve economics, token supply, or the LPFeeVault.
...
/// `CurveDefaults` + `curveDefaults()` (seam; LAUNCH-2), a pure view that
/// changes no existing signature. Config mutability model (contracts.md):
...
interface ICurveFactory {
    ...
    /// constant because the curve takes no constructor args (contracts.md pattern
    ...
    struct CurveParameters {
        ...
        address creator; // token creator — the creator-fee-leg beneficiary
        address creatorVault; // pull-payment CreatorVault the curve sweeps the creator leg to
        ...
        uint256 graduationEth; // net-of-fee real-reserve threshold
        uint16 tradeFeeBps; // treasury leg, snapshot; tradeFeeBps + creatorFeeBps ≤ MAX
        uint16 creatorFeeBps; // creator leg, snapshot. 0 in v1/mainnet; testnet 50
        uint256 graduationFee; // flat, → treasury first at migration (step 1)
        uint256 callerReward; // permissionless graduate() incentive
        uint64 earlyWindowSeconds; // anti-sniper timestamp window
        uint128 maxEarlyBuyWei; // per-tx gross ETH cap inside the window
    }

    /// @notice Immutable factory-level curve-shape defaults — the five M0 `constants.json`
    /// economics snapshotted into every FUTURE curve at creation (
    /// seam; contracts.md). Returned by {curveDefaults} so pre-create consumers
    ...
    /// outside `createToken` — and (b) the per-curve public immutables, which only
    ///      exist after a curve deploys. All five fields are `immutable` on the factory: a new
    /// curve shape = a new factory version, so this view can never change.
    struct CurveDefaults {
        ...
        uint256 graduationEth; // net-of-fee real-reserve graduation threshold
    }

    /// @notice Live config snapshot for Router/UI (contracts.md `config()`).
    /// @dev Field set derived from the storage table (owner-settable values + pause flags +
    ...
    struct FactoryConfig {
        ...
        uint16 creatorFeeBps; // creator leg : 0 default; configurable, additive under the cap
        uint256 creationFee; // flat, → treasury
        ...
        uint128 perTokenEthCap; // beta cap, buy-side only (gate 7)
    ...
    /// @notice Admin setter events — every owner setter emits (contracts.md).
    event TreasuryUpdated(address indexed newTreasury);
    event TradeFeeUpdated(uint16 newBps);
    /// @notice Creator-fee leg (bps) default for future curves updated.
    event CreatorFeeUpdated(uint16 newBps);
    /// @notice One-time CreatorVault wiring — the pull-payment sink for the creator leg.
    event CreatorVaultSet(address vault);
    /// @notice One-time LPFeeVault wiring — the registered post-graduation ERC20 fee
    ...
    ///. Only callable by the Router (which collects the creation fee and
    ...
    /// (contracts.md, proof-by-construction obligation b).
    ...
    /// (constant-init-code CREATE2 staging pattern, contracts.md).
    ...
    /// the per-curve public immutables once a curve exists. Behavior is
    ...
    /// @notice True iff `account` is a factory-deployed curve (migrator gate, contracts.md).
    function isCurve(address account) external view returns (bool);

    /// @notice Current config snapshot for Router/UI (contracts.md).
    ...
    /// @notice Trade-fee hard cap, 200 bps = 2% (constant, contracts.md).
    function MAX_TRADE_FEE_BPS() external view returns (uint16);

    /// @notice One-time-set Router address (contracts.md storage).
    ...
    /// @notice Fee destination — Gnosis Safe. Read live by curves.
    function treasury() external view returns (address);

    /// @notice Pull-payment CreatorVault, one-time-set. address(0) until wired;
    ...
    /// @notice The registered LPFeeVault of this generation, one-time-set. Gates
    ...
    /// @notice Flat creation fee, collected by the Router (contracts.md).
    function creationFee() external view returns (uint256);

    /// @notice Launch pause flag. Blocks creates only.
    function pauseCreates() external view returns (bool);

    /// @notice Buy pause flag. Blocks buys only — never sells, graduate, or collect.
    function pauseBuys() external view returns (bool);

    /// @notice Per-token beta ETH cap (gate 7), buy-side only.
    function perTokenEthCap() external view returns (uint128);

    /// @notice Global beta ETH cap (gate 7), buy-side only.
    function globalEthCap() external view returns (uint128);

    // ─────────────── Owner (Safe) — hard-capped in code (contracts.md) ───────────────
    ...
    /// @dev Treasury leg. Re-asserts `newBps + creatorFeeBps ≤ MAX_TRADE_FEE_BPS` (200, the
    ///      ≤2% cap); applies to FUTURE curves only.
    function setTradeFeeBps(uint16 newBps) external;
    /// @dev Creator leg. Re-asserts `tradeFeeBps + newBps ≤ MAX_TRADE_FEE_BPS`
    ...
    /// @dev ONE-TIME: reverts AlreadyInitialized if already set; non-zero.
    function setCreatorVault(address vault) external;
    /// @dev ONE-TIME: reverts AlreadyInitialized if already set; non-zero. Wires the
    ...
    /// @dev Future curves only (anti-sniper defaults).
    function setAntiSniper(uint64 windowSeconds, uint128 maxEarlyBuyWei) external;
    /// @dev Beta caps; lift = set to type(uint128).max (M5).
