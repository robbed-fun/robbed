...
/// @title IRouter — single user entrypoint (contracts.md)
...
/// Fees are NEVER caller-supplied — no fee parameter exists in any signature.
/// BOTH ETH-leg fees (treasury + Phase-2 creator) are computed and accrued
...
/// (contracts.md). Slippage + deadline on every trade path incl. create's atomic buy
///.
interface IRouter {
    /// @notice One-tx launch: create token + curve + pre-initialized V3 pool, optionally atomic
    /// initial buy with the remaining msg.value (anti-self-snipe).
    ...
    /// via the factory's TokenCreated. The creator's initial buy IS subject to
    /// the anti-sniper cap — no carve-out (contracts.md).
    ...
    /// is already sufficient (contracts.md). `deadline` doubles as the permit
    ...
    /// @notice Proxy to curve.quoteBuy (contracts.md).
    ...
    /// @notice The CurveFactory (sole storage, immutable — contracts.md).
