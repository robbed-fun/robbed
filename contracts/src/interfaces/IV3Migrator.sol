...
/// @title IV3Migrator — graduation executor, Option B (spec §6.3, §6.3.2, §12.1;
///        contracts.md §2.5)
...
///         defense). No owner.
/// @dev FROZEN interface (tests-as-spec phase). Invariants owned (gate 2, contracts.md §2.5):
...
///      spec §13 O-8); donation/sync-style/swap griefing cannot produce a hostile-ratio mint
///      (corrected or reverted); `Graduated` emitted exactly once per token.
interface IV3Migrator is IUniswapV3SwapCallback {
    // ────────────────────────────────── Events ─────────────────────────────────

    /// @notice Emitted on creation-time pool initialization (spec §6.3.2).
    ...
    /// @notice Canonical graduation event (spec §6.3 step 5, §12.15 event family).
    event Graduated(
    ...
    ///         pool at the deterministic graduation price (spec §6.3.2).
    ...
    /// @dev onlyCurve (factory.isCurve(msg.sender)). Full sequence (contracts.md §3.4):
    ...
    ///      0x…dEaD, WETH dust → treasury (spec §12.13); emit Graduated. Reverts propagate to
    ...
    /// @notice V3 fee tier: 10_000 (1%, spec §12.1; constant).
