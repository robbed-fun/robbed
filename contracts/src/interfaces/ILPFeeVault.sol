...
/// ) and `registerCreator(tokenId, creator)` (migrator-only, set-once — binds the
...
/// property). Implementation target ~50 lines (grows for the split + registration,
/// — documented in the source).
/// @dev Copy language everywhere (as amended by) "LP principal permanently locked;
...
interface ILPFeeVault is IERC721Receiver {
    ...
    /// per leg (contracts.md).
    event FeesCollected(uint256 indexed tokenId, uint256 amount0, uint256 amount1);

    /// @notice Emitted on every collect with the per-leg 50/50 split (— two
    ...
    /// @notice Emitted when the migrator binds a graduated position's creator.
    ...
    /// to the pull-payment {ICreatorVault} credited to `creatorOf[tokenId]`.
    ...
    /// @notice Migrator-only, set-once: bind `tokenId → creator` at graduation. The
    ...
    /// @notice Gnosis Safe treasury, fixed at deploy, unchangeable forever.
    ...
    /// @notice Creator share of every post-graduation collect, in bps (immutable 5000 = 50%).
