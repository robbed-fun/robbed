...
/// @title LaunchToken — ROBBED_ launch token (contracts.md)
...
/// "ownerless"; "owner can never touch token economics"). Proven by the unit test
...
/// no public mint/burn; we add none. Graduation "dust burn" is a plain
///         `transfer` to `0x…dEaD` performed by the migrator — the token itself is burn-free, so
/// `totalSupply()` is constant `1e27` forever (contracts.md invariant). Proven by unit
...
contract LaunchToken is ERC20, ERC20Permit, ILaunchToken {
    ...
    /// @dev `constant` (never an SSTORE target) — the fixed 1,000,000,000e18 supply (,
    /// contracts.md). `CURVE_SUPPLY + LP_TOKEN_TRANCHE == TOTAL_SUPPLY` is asserted by the
